	...
	foreach($pregunta as $preguntas){
		if($contador_numero_de_pregunta > $total_preguntas){
			...
			//if(empty($examenes_rptas)){
				...
				// 2024 - registro cada desarrollo de cada examen independiente 
				...
				$campos1 = array('id_sxe',array('id_suscrito',$suscrito),array('id_examen',$examen),array('id_pregunta',$pregunta),array('fecha_registro',$fecha),array('fecha_actualizacion',$fecha),array('orden',$orden),array('estado_idestado',1));


				
				if( !empty($preg) ){
					$campos1= array_merge( $campos1 , array( array('id_rpta',$preg)) );
				} // sino se guarda nulo q no marco respuesta 
...
$total_minu = ($res_h*60) +  intval($res_m);
$min = $minuto2 - $total_minu;

// normalmente se actualizaba 
...
$result = $ulr_resul.'/resultado'.'/'.$id_intento;
