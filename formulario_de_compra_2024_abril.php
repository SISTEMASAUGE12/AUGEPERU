<?php 
$img_generica="img/imagen_generica.jpg";
...
if( !empty($curso[0]['cursos_especialidades']) ){ 
	...
	if( !empty($especiales) ){
		...
		foreach( $especiales as $anexo_especiales ){
			...
			$imagen_xx= !empty($anexo_especiales["imagen2"])?"tw7control/files/images/capa/".$anexo_especiales["imagen2"]:$img_generica;
			$_data_cursos_especiales.="<div class='detalle'><figure class='img-cesta rel'><img src='".$imagen_xx."'><div class='capa abs'></div></figure><div class='delete_curso text-center '></div><div class='titulo'><p class='poppi-b color1'>".$anexo_especiales["codigo"].' - '.$anexo_especiales["titulo"]."</p></div></div>";	
			$n_esp++;   
...
if( !empty($curso[0]['cursos_dependientes']) ){
	...
	if( !empty($dependientes) ){
		foreach( $dependientes as $anexo ){
			$imagen_yy= !empty($anexo["imagen2"])?"tw7control/files/images/capa/".$anexo["imagen2"]:$img_generica;

			$_data_cursos_dependientes.="<div class='detalle'><figure class='img-cesta rel'><img src='".$imagen_yy."'><div class='capa abs'></div></figure><div class='delete_curso text-center '></div><div class='titulo'><p class='poppi-b color1'>".$anexo["codigo"].' - '.$anexo["titulo"]."</p></div></div>";			
					...
					if( !empty($anexo['cursos_especialidades']) ){ 
							...
							if( !empty($especiales_dentro_de_packs) ){
								...
								foreach( $especiales_dentro_de_packs as $anexo_especiales ){
									...
									$imagen_zz= !empty($anexo_especiales["imagen2"])?"tw7control/files/images/capa/".$anexo_especiales["imagen2"]:$img_generica;

									$_data_cursos_especiales.="<div class='detalle'><figure class='img-cesta rel'><img src='".$imagen_zz."'><div class='capa abs'></div></figure><div class='delete_curso text-center '></div><div class='titulo'><p class='poppi-b color1'>".$anexo_especiales["codigo"].' - '.$anexo_especiales["titulo"]."</p></div></div>";	
									$n_esp++;   
...
$imgproduct= !empty($curso[0]['imagen2'])?"tw7control/files/images/capa/".$curso[0]['imagen2']:$img_generica;
...
// if( $curso[0]["modalidad"]==2 && !empty($curso[0]["fecha_inicio"]) ){  
	...
	/** algoritmo de mostrar fech actual -req. abril 2024  */	
	// echo  $_mes_actual_hoy= date("n",strtotime(fecha_hora(1))); 
	// echo  $_mes_actual_fecha_sistema= date("n",strtotime($curso[0]["fecha_inicio"]) ); 
	// echo  fecha_hora(1) ;
/*
*** crear algoritmo
inicio hoy 21 de abril [ que salga por dia ]-
1. muestro fecha de inciio del sistema 
2. - si lo pasa y esta dentro del mes mostrar la fecha actual  hoy 19 de marzl 
3. y si pasa el mes actual ya deja la fecha de inicio registrado .
*/

	if( $curso[0]["fecha_inicio"] >= fecha_hora(1) ){
		$fecha_inicio_valor = $curso[0]['fecha_inicio']; 
		$texto_intro_fecha= "INICIA TU PREPARACIÓN";

	}else if( $curso[0]["fecha_inicio"] < fecha_hora(1) ){  // si e smenor a fecha actual 
		// valido si el dia actual esta en el rango del mes actual a la fecha registrada en el sistema::: 
		if( $_mes_actual_fecha_sistema == $_mes_actual_hoy ){ // indicacion::2 
			$fecha_inicio_valor = fecha_hora(1); 
			$texto_intro_fecha= "INICIA HOY TU PREPARACIÓN";
			//echo "mod::2";
		}else{  // indiciaon::3, aca ya vencio la fecha de nicio y mes 
			//echo "mod::3";
			$texto_intro_fecha= "INICIA ";
			$fecha_inicio_valor = $curso[0]['fecha_inicio']; 
		}	
	}
...
//	$fecha_inicio_valor = $curso[0]['fecha_inicio']; 
	...
	$fecha_inicio_curso= strtr(date('d\ \d\e\ M',strtotime($fecha_inicio_valor)),$meses);

?>	
	<div class="fecha_en_vivo blanco text-center <?php  echo (  $_GET["rewrite"] == "libros")?' hide ':''; ?> ">
		<p class="poppi ">
			<small> <?php echo $texto_intro_fecha; ?></small> 
			<?php echo $fecha_inicio_curso; ?>  <span style="padding-left:10px;"></span> <?php echo $curso[0]['hora_inicio']; ?>			
		</p>
	</div>
<?php 
// }  // si en modalidad en vivo   ?>
	...
	<ul class="no-bullet poppi ">
		...
		<li class="texto">
			...
			Clases  <b> <?php echo ($curso[0]["modalidad"]==1)?'GRABADAS':' EN VIVO'; ?> </b>
		</li>									
		...
		<?php }elseif($_GET['rewrite'] =='libros'){ // si e slibro smuestro esto ?> 				
			<li class="texto"><div class="img"><img class="verticalalignmiddle" src="img/iconos/icono_formualrio_10.png"></div> <b>  COMPLEMENTAR  </b>  Conocimientos adquiridos </li>				
			<li class="texto"><div class="img"><img class="verticalalignmiddle" src="img/iconos/icono_formualrio_11.png"></div> <b> MEJORAR  </b> Desempeño en el aula. </li>				
			<li class="texto"><div class="img"><img class="verticalalignmiddle" src="img/iconos/icono_formualrio_12.png"></div> <b>  ESTRATEGIAS </b> Para afrontar los examenes.. </li>				
			<li class="texto"><div class="img"><img class="verticalalignmiddle" src="img/iconos/icono_formualrio_13.png"></div> <b>  LOGRAR </b> Comprensión de categorías pedagógicas. </li>				
...
<?php 
		...
		$boton_comprar_html='
		<button class="poppi  boton hola '.(  ( isset($_SESSION["suscritos"]["id_suscrito"]) && ( $pagina_open !="_detalle_flotante" ) )?'  compra_directa ':'' ).'  estoy_detalle_curso   ">
