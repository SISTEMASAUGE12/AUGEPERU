...
if(isset($_GET['task3']) && ($_GET['task3']=='resultado')){   // solucion del examen 
	$exa = executesql("SELECT sxe.ide, e.*,ce.titulo as cate_exa FROM suscritos_x_examenes sxe INNER JOIN examenes e  ON e.id_examen=sxe.id_examen INNER JOIN categoria_examenes ce ON e.id_cate = ce.id_cate  WHERE e.estado_idestado=1 and sxe.estado_idestado=1 and e.titulo_rewrite='".$_GET['task2']."' ORDER BY sxe.ide desc LIMIT 0,1	");
?>
<main id="examen">
<?php   
...
?>
	<div class="callout callout-2"><div class="row">
    	...
    	<div class="large-12 columns"><div class="detalle_resultado">
    		<div class="hea-prin">
    			...
    			<a onclick="resumen_examen('<?php echo $exa[0]['titulo'] ?>',<?php echo $rpta_t[0]['total_preguntas_registradas'] ?>,<?php echo $rpta_c[0]['total'] ?>, '<?php echo $porcentaje; ?>', <?php echo !empty($rpta_c[0]['punto']) ? $rpta_c[0]['punto'] : '0' ?>,<?php echo $rpta_t[0]['punto'] ?>,'<?php echo $min_exa[0]['minutos'] ?>')">Ver Resumen</a>
    		</div>
    		<?php 
    		$rpta_c = executesql("SELECT COUNT(r.id_rpta) as total, SUM(e.puntos) as punto 
		FROM respuestas r 
		INNER JOIN preguntas e ON r.id_pregunta = e.id_pregunta 
		INNER JOIN suscritos_x_examenes_rptas ser ON r.id_rpta = ser.id_rpta 
		WHERE r.estado_idestado=1 and ser.id_suscrito=".$_SESSION["suscritos"]["id_suscrito"]." 
		...
		ORDER BY ser.id_rpta desc LIMIT 0,1");
    		...
    		$min_exa = executesql("SELECT minutos FROM suscritos_x_examenes WHERE id_suscrito=".$_SESSION["suscritos"]["id_suscrito"]." AND id_examen=".$exa[0]['id_examen']);
    		$porcentaje = round(($rpta_c[0]['total']*100)/$exa[0]['total_preguntas'],2);
    		?>
    <script src="js/sweetalert.min.js"></script>
    		<script type="text/javascript">
				...
				window.onload = resumen_examen('<?php echo $exa[0]['titulo'] ?>',<?php echo $rpta_t[0]['total_preguntas_registradas'] ?>,<?php echo $rpta_c[0]['total'] ?>, '<?php echo $porcentaje ?>', <?php echo !empty($rpta_c[0]['punto']) ? $rpta_c[0]['punto'] : '0' ?>,<?php echo $rpta_t[0]['punto'] ?>, '<?php echo $min_exa[0]['minutos'] ?>');
			</script>
    		<div class="deta-prin">
...
?>
    				...
    				<div class="preguntas" id="pregunta<?php echo $contador_numero_de_pregunta; ?>" style="display:none">
    					...
    					<?php /*if(!empty($preguntas['descripcion'])){*/ ?>
