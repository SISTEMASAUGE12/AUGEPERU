...
<div class="curso-list">
    ...
    <figure class="rel ">
			...
			-->
			<img src="<?php echo !empty( $detalles['imagen2'])?$imgproduct:'img/no_imagen.jpg'; ?>" class="principal " alt="">
			<a class="abs mpopup-03 " title="ver más" href="detalle/<?php echo $detalles['tiprewri'].'/'.$detalles['titulo_rewrite'] ; ?>"><img src="img/iconos/ico-play-sombra_2024.png" alt="ver video" class="verticalalignmiddle"></a>
...
?>
    <div class="deta">
        ...
        <h2 class="color1 poppi">
					<small  class=" lleva_codigo   "><?php echo $detalles['codigo']; ?></small>
        ...
        <p class="texto poppi-l breve_detalle "><?php echo short_name($detalles['breve_detalle'],90) ?></p> 
				...
				<?php if( !empty($profe)){  ?>
				<ol class="no-bullet poppi-l  hide ">
...
$precio_online=  $precio; // pago tarjeta y pago deposito van a enjar e mismo precio. 
			...
			if($detalles["tipocurso"]== $_variable_tipo_categoria_gratuita ){ //id de tipo gratuitos
				//  echo '<blockquote class="poppi-b color2">Gratuito</blockquote>'; 
				 echo '<blockquote class="poppi-b color2">Inicia Hoy</blockquote>'; 
        ...
        <blockquote class="poppi-s color1 ">
            ...
            <?php  echo ($detalles['costo_promo']>0) ? '<small class="poppi rojo"> s/ '.$detalles['precio'].'</small>' : 's/ '.$detalles['precio']; ?>
			 ...
			 <?php	/* fecha limite de la oferta */ 
			 	/* cronometro promo ocultado marzo-2024  */
			 ?>
						<span style="list-style:none;" class="color2 cronometro    hide " style="font-size: 12px;"><div class="img"><img class="" src="img/iconos/reloj.png" alt="oferta vence en"></div>Oferta hasta <?php echo $detalles["fecha_fin_promo"]; ?> <div id="countdown_<?php echo $detalles["id_curso"] ?>" style="display:inline-block;padding-left:7px;"></div></span>
						<script>
		...
		<?php	}else{ // disponible para la compra .. 
						...
						if($detalles["tipocurso"]== $_variable_tipo_categoria_gratuita){ //id de tipo gratuitos 
							 // curso es gratuito 
								...
								echo '<a href="'.$link.'" class="boton poppi-sb btn_grauito " title="suscibete"><img src="img/iconos/carrito.png" alt="agregar al carrito"> Comprar s/ '.$precio.'</a>';
						 
						}else{ 
					?>
					<a href="<?php echo $link ?>" title="agregar al carrito" class="boton poppi-sb"><img src="img/iconos/carrito.png" title="cesta carrito">Comprar s/ <?php echo $precio; ?> </a>
<!-- 				
...
<?php 			} // si no es gratuito 
			...
			if($detalles["tipocurso"]== $_variable_tipo_categoria_gratuita  ||  !empty($detalles['cursos_especialidades']) ){ //id de tipo gratuitos 
				if($detalles["tipocurso"]== $_variable_tipo_categoria_gratuita){  //id de tipo gratuitos 
					...
					echo '<a href="'.$link.'" class="boton poppi-sb  " title="agregar al carrito"> <img src="img/iconos/carrito.png" alt="agregar al carrito">Comprar  s/ '.$precio.'</a>';
					
				}			 
			}else{ 
 ?>
					<a href="<?php echo $link ?>" title="ver mas detalles" class="boton poppi-sb"><img src="img/iconos/carrito.png" alt="carrito">Comprar s/ <?php echo $precio; ?></a>
					<!-- 
					<form id ="ruta-<?php echo $detalles["id_curso"]; ?>" class="add" method="POST" action="process_cart/insert.php" accept-charset="UTF-8">
						<input type="hidden" name="id"  value="<?php echo $detalles["id_curso"] ?>">
						<input type="hidden" name="id_tipo"  value="<?php echo $detalles["id_tipo"]; ?>">
