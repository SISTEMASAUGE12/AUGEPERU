	...
	if ( !empty($_POST["fechabus_1"]) && !empty($_POST["fechabus_2"]) ){
		...
		if (file_exists ("bbva_".$archivo)){ 
			...
			for ($i=1;$i<=$filas;$i++){  // $i numero de linea de donde desea empiece el recorrido 
				...
				$_DATOS_EXCEL[$i]['detalle'] = $objPHPExcel->getActiveSheet()->getCell('E'.$i)->getCalculatedValue();
				...
				$_DATOS_EXCEL[$i]['importe'] = $objPHPExcel->getActiveSheet()->getCell('F'.$i)->getCalculatedValue();
			...
			$sql_resagados_de_bd=" select v.*, u.nomusuario as usuario FROM vouchers v 
														...
														WHERE v.estado_idestado=1 and v.id_banco=9 and  DATE(v.fecha_registro)  BETWEEN  DATE('".$_POST['fechabus_1']."')  and DATE('".$_POST['fechabus_2']."') 
			...
			if( !empty($restantes_del_sistema) ){
				foreach($restantes_del_sistema as $no_excel){
					$data_sistema_no_en_excel.= "<tr><td>BBVA</td> <td>".$no_excel['fecha_registro']."</td><td>".$no_excel['id_pedido']."</td><td>".$no_excel['codigo_operacion']."</td><td>".$no_excel['total']."</td><td>".$no_excel['usuario']."</td></tr>";
				}
			}
			...
			<div class='col-xs-12'>
				<div class='form-group'><strong><center>
					...
					<form action='val_excel_bbva_1_excel_no_encontrado_en_sistema.php' method='post'> ";												
		...
		echo "	<span style='color:red;'> VOUCHERS NO ENCONTRADOS DEL EXCEL EN SISTEMA: ".$no_encontrados_excel."
					...
					<form action='val_excel_bbva_2_del_sistema_no_encontrado_en_excel.php' method='post'> ";
