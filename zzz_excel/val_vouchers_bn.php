...
<h2 style="padding-top:10px;"> 
	<b>Vouchers BN Corriente </b>: <?php echo fecha_hora(1); ?>  </br>
...
if (isset($action)== "upload"  ){
	if ( !empty($_POST["fechabus_1"]) && !empty($_POST["fechabus_2"]) ){
		...
		$destino = "bn_corriente_".$archivo;//Le agregamos un prefijo para identificarlo el archivo cargado
		...
		if (file_exists ("bn_corriente_".$archivo)){ 
			/** Llamamos las clases necesarias PHPEcel */
			require_once('classes/PHPExcel.php');
			require_once('classes/PHPExcel/Reader/Excel2007.php');					
			// Cargando la hoja de excel
			$objReader = new PHPExcel_Reader_Excel2007();
			$objPHPExcel = $objReader->load("bn_corriente_".$archivo);
			...
			for ($i=1;$i<=$filas;$i++){  // $i numero de linea de donde desea empiece el recorrido 
				$_DATOS_EXCEL[$i]['fecha_banco'] = $objPHPExcel->getActiveSheet()->getCell('B'.$i)->getCalculatedValue();
				...
				$_DATOS_EXCEL[$i]['importe'] = $objPHPExcel->getActiveSheet()->getCell('G'.$i)->getCalculatedValue();
				$_DATOS_EXCEL[$i]['codigo_operacion'] = $objPHPExcel->getActiveSheet()->getCell('D'.$i)->getCalculatedValue();
			...
			foreach($_DATOS_EXCEL as $campo => $valor){   // recorremos excel y sus valores 
				...
				if ( $_POST['importe'] >0 && !empty($_POST["codigo_operacion"]) ){ 
					...
					$sql_validate="select * from vouchers WHERE id_banco='1' and  DATE(fecha_registro)  BETWEEN  DATE('".$_POST['fechabus_1']."')  and DATE('".$_POST['fechabus_2']."')  and codigo_operacion='".$codigo_operacion."' order by id_vouchers desc  ";
			...
			$sql_resagados_de_bd=" select v.*, u.nomusuario as usuario FROM vouchers v 
														...
														WHERE v.estado_idestado=1 and v.id_banco=1 and  DATE(v.fecha_registro)  BETWEEN  DATE('".$_POST['fechabus_1']."')  and DATE('".$_POST['fechabus_2']."') 
			...
			if( !empty($restantes_del_sistema) ){
				foreach($restantes_del_sistema as $no_excel){
					$data_sistema_no_en_excel.= "<tr><td>BN CORRIENTE</td> <td>".$no_excel['fecha_registro']."</td><td>".$no_excel['id_pedido']."</td><td>".$no_excel['codigo_operacion']."</td><td>".$no_excel['total']."</td><td>".$no_excel['usuario']."</td></tr>";
				}
			}
			...
			echo "
				...
				<div class='col-xs-12'>
					<div class='form-group'><strong><center>
						...
						<form action='val_excel_bn_corriente_1_excel_no_encontrado_en_sistema.php' method='post'> ";												
			...
			echo "	<span style='color:red;'> VOUCHERS NO ENCONTRADOS DEL EXCEL EN SISTEMA: ".$no_encontrados_excel."
						...
						<form action='val_excel_bn_corriente_2_del_sistema_no_encontrado_en_excel.php' method='post'> ";
			...
			//Borramos el archivo que esta en el servidor con el prefijo bn_corriente_
			unlink($destino);
						
			
		}else{	//si por algun motivo no cargo el archivo bn_corriente_ 
