function autocompletar_certificado_venta(tipo_curso) {
	...
	if (nombre.length > min_length) {
		let _url='proceso_busqueda_certificados_para_vender.php';
		...
		$.ajax({
			...
			success:function(data){
				$('#listadobusqueda_certificados_a_vender').show();//mistrar la lista
				$('#listadobusqueda_certificados_a_vender').html(data);//mostrar resultado de consulta en la lista
			}
		});
	} else {
		$('#listadobusqueda_certificados_a_vender').hide();
	}
}
...
function seleccionar_add_certificado_venta_manual(id,nombre,tipo){    
	...
	let ides_dependientes_string= $('input[name="array_certificados_a_vender[]"]').val();
	...
	//alert('-->'+fila_curso);
			...
			if(nro == sino_suplicado ){ // sino tiene duplicados .. agregamos el id_curso al aarray[]
				...
				// $('#array_certificados_a_vender').val(ides_dependientes);	// asignamos el nuevo valor del array al input ..
				$('input[name="array_certificados_a_vender[]"]').val(ides_dependientes);
...
function quitar_dependiente(id_quitar){
	// setear valor al imput id y nombre	
		let ides_dependientes_que_ya_tiene= $('input[name="array_certificados_a_vender[]"]').val();
	...
	// alert(ides_dependientes);
	$('#array_certificados_a_vender').val(ides_dependientes);		
}