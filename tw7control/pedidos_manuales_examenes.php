...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  $bd=new BD;
	...
	$_POST['estado_pago']=1;
	$_POST['categoria_venta']=6; // categoria venta manual de examen 
  ...
  if($_GET["task"]=='insert'){
			...
			if(isset($_POST['subcategorias'])){ // del slect multiple 
				foreach($_POST['subcategorias'] as $v){
					...
					$_POST["id_examen"] = $division[1];
					$n_cursos++;

					$sql_valida_si_curso="select * from suscritos_x_examenes where id_suscrito='".$_POST["id_suscrito"]."' and id_examen ='".$_POST["id_examen"]."' and estado!=3 ";
					...
					if( !empty($validamos_si_ya_tiene_curso) ){  // si ya tiene un curso, blokeamos la venta 
						...
						$_yacomprados.=$_yacomprados.' ,'.$_POST["id_examen"];
					}
				}
			...
			/*
			if(isset($_POST['array_examenes_a_vender'])){
				$data_string= implode(',',$_POST['array_examenes_a_vender']);  // como llega en una linea, lo comnvierto a string 
			...
			*/
			...
			if($_curso_repetidos == 0){ // si aun no tiene el curso, le vendemos 
						...
						if($id_pedido > 0){
								...
								if(isset($_POST['subcategorias'])){
									foreach($_POST['subcategorias'] as $v){
										...
										$_POST['talla']='7777'; // examen vendido directamnete
										
										include('inc_proceso_recorrido_linea_pedido_y_registros_examen_vendido.php'); // aca se ejecutan todos los procesos 
				...
				echo "<script>alert(' Seleecciona al menos  un examen a vender! ');</script>";
			}

		}else{
			/* compelta todos los datos del cleinte para poder venderle  */
			echo "<script>alert('Venta no procesada: ".$_curso_repetidos." Cliente tiene el/los examen ya comprados: ".$_yacomprados."');</script>";
		}
...
<script type="text/javascript" src="js/buscar_examen_para_vender.js?ud=<?php echo $unix_date; ?>"></script>
    ...
    <div class="col-md-12">
          ...
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">VENTA DE EXAMEN MANUAL</h3>
            </div><!-- /.box-header -->
...
?>
            ...
            <!-- form start -->
            <form  action="pedidos_manuales_examenes.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>"   class="form-horizontal" method="POST" enctype="multipart/form-data" autocomplete="OFF" onsubmit="return aceptar()" >
<?php 
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-sm-4 ">
                            <?php create_input("text","total",$usuario["total"],"form-control",$table,' required ',$agregado); ?>
											...
											<div class="col-sm-12">
                        <div class="form-group">
                          <label for="inputPassword3" class="col-sm-2 control-label">#Total de examenes:</label>
                          <div class="col-sm-4 ">
                    ...
                    <div id="collapseTwo" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingTwo">
                      <div class="panel-body">
									...
									<div class="form-group">																		
										...
										<label for="inputEmail3" class="col-sm-4 control-label" style="float:none;text-align:left;">Selecciona los examenes a vender:</label>
										<label for="inputEmail3" class="col-sm-12 control-label" style="float:none;text-align:left;"></br> <small>Mantener presionada la tecla "ctrl" para seleccionar varias examenes, esto dandoles click en el nombre </small> </br></br></label>
										...
										<div class="col-sm-6  ">
											<?php 
												...
												$sql= "SELECT c.id_examen ,CONCAT(c.id_examen,' - ',c.titulo) as examen, cat.id_cate,cat.titulo as categ 
												FROM examenes c
													LEFT JOIN categoria_examenes cat ON cat.id_cate=  c.id_cate 
													WHERE c.privacidad=3 and c.estado_idestado=1 
													GROUP by c.id_examen 
													ORDER BY c.id_examen desc 
												";																
													// group by ,
									...
									<?php 
													...
													foreach($certificados_x_cursos as $row) $grp1[$row['categ']][$row['id_examen'].'_'.$row['id_examen']] = $row['examen']; $modalidad=1;
													...
													foreach($curso_comprado as $row) $grp2[] = $row['id_examen'];
								...
								<?php 
													...
													echo "<p style='color:red;'>No hay examenes disponibles para ser adquiridos. </p>";
						...
						<div class="form-group" style="  padding-left:40px;" >
							...
							<div class="form-group" style="   " >
								<div class="col-sm-10 ">
									...
									<ul id="listadobusqueda_examenes_a_vender" class="no-bullet"></ul>
								</div>
							</div>
								...
								<div class="col-sm-12 " style="background: #ddd;padding: 5px 10px 10px;border-radius: 6px;">
									<div class="col-sm-3">
										<label for="inputPassword3" class=" control-label">Tipo:</label>                                 
                              			<input name="array_examenes_a_vender[]" id="array_examenes_a_vender" type="hidden">		
									</div>					
									<div class="col-sm-7">
...
<?php 
if($_GET["task"]=='edit' ){ 
$detallepro=executesql("select p.*,  sxc.*  from linea_pedido p
									LEFT JOIN suscritos_x_examenes sxc ON p.id_pedido = sxc.id_pedido 
            ...
            <?php foreach($detallepro as $rowdetalle){ ?>
                            <tr id="order_<?php echo $rowdetalle["id_linea"]; ?>">
                              <td ><?php
																	//  echo ($rowdetalle["talla"]=='7777')?'Certificado': '-- '; 
																	echo "Examen";
															?></td>
                              <td ><?php echo $rowdetalle["id_examen"]; ?></td>  <!--  ID CERTIFICADO -->
                              <td >
									<?php $certificadox= executesql("select * from examenes where id_examen='".$rowdetalle["id_examen"]."' ");
...
<script>
var link = "pedidos_manuales_examene";/*la s final se agrega en js fuctions*/
...
var ar = "al";
var id = "id_pedido";
var mypage = "pedidos_manuales_examenes.php";
</script>
  ...
  foreach($usuario as $reg => $item)
    ...
    $state = 1;
		...
		if( !empty($_GET['id_pedido'] ) && $_GET['id_pedido'] > 0 ){ // si existe id_pedido hacemos el recorrido 
			$bd=new BD;  
			$sql_p='select ide, estado from suscritos_x_examenes  where  id_pedido="'.$_GET["id_pedido"].'" ';
...
// rechazamos pago
  ...
  if(!empty($usuario) && !empty($_GET['comentario']) ){   // comentario motivo de rechazo obligatorio 
				...
				$sql_cer='select ide, estado from suscritos_x_examenes  where  id_pedido="'.$_GET["id_pedido"].'" ';
  ...
  LEFT JOIN bancos ban ON pp.banco_pago=ban.id_banco   
	WHERE pp.tipo_pago='".$_GET["tipo_pago"]."' and pp.categoria_venta=6  ";   // catgoria de venta 6:: venta mahual de examens 
 ...
 // echo  $sql; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="pedidos_manuales_examenes.php";
?>
...
<?php
	// $line=executesql("SELECT c.titulo, c.codigo FROM linea_pedido lp INNER JOIN cursos c ON lp.id_curso = c.id_curso WHERE lp.id_pedido = '".$detalles['id_pedido']."' ORDER BY lp.orden DESC");
	  ...
	  $sql_linea="SELECT cer.titulo as titulo_curso, cer.id_examen, cer.titulo as titulo_certificado,  cer.id_examen as codigo FROM suscritos_x_examenes lp INNER JOIN examenes cer ON lp.id_examen=cer.id_examen      WHERE lp.id_pedido = '".$detalles['id_pedido']."' ORDER BY lp.ide DESC";

	$line=executesql($sql_linea);
	if(!empty($line)){ 
...
?>
	<tr>
		...
		<td> <b> EXAMEN: </b> </td>
		<td>  <?php echo '<b>Cod: '.$linea["id_examen"].' </b>  - '. $linea["titulo_certificado"]; ?></td>
		<td> <b> #  </b></td>
		<td> 1</td>
		<td></td>
		<td></td>
...
<script>
$(function(){
  // reordenar('pedidos_manuales_examenes.php');
  // checked();
  // sorter();
});
</script>
