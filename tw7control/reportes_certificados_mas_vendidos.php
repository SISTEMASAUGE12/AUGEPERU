...
if($_GET["task"]=='finder'){
  ...
  $sql= "SELECT count(ce.id_certificado) c, SUM(lp.subtotal) total_venta, lp.*,YEAR(lp.fecha_registro) as anho, MONTH(lp.fecha_registro) as mes, e.nombre AS estado ,s.email as email,  CONCAT(s.nombre,' ',s.ap_pa,' ',s.ap_ma )as suscritos , s.dni as dni, s.telefono, c.codigo, c.titulo, pp.codigo as codped, pp.tipo_pago, ce.id_certificado, ce.titulo as certificado, ce.precio as precioc 
  ...
  WHERE lp.talla = '9999'  AND pp.estado_pago = '1' ";
  
  if (isset($_GET['criterio_mostrar'])) $porPagina=$_GET['criterio_mostrar'];
  if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per'])){
    $stringlike=fn_filtro(substr($_GET['criterio_usu_per'], 0, 16));
			$pos = strpos($sql, 'WHERE');
      $sql.= (($pos === false) ? "WHERE" : "AND" )." (ce.id_certificado LIKE '".$stringlike."%' )"; 
  }else{
 ...
 $sql.= " GROUP BY ce.id_certificado HAVING c >= 1 ORDER BY total_venta DESC";
 ...
 // echo $sql; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="reportes_certificados_mas_vendidos.php";
?>
  <table id="example1" class="table table-bordered table-striped">
		...
		<a href="javascript:fn_exportar('tipo_form=1&fecha_filtro_inicio=<?php echo $fecha_excel_inicio;?>&fecha_filtro_fin=<?php echo $_GET["fechabus_2"];?>','certificados_mas_vendidos');" class="btn btn-primary excel "  > Excel</a>
...
?>
          ...
          <th class="sort ">Total de ventas</th>
...
?>        
       <tr>
        ...
        <td class="cnone"><?php echo $detalles["total_venta"]; ?></td>
            ...
            <form action="javascript: fn_buscar();" id="frm_buscar" name="frm_buscar">
              <div class="bg-gray-light">
								<div class="col-sm-12 titulo_reporte criterio_buscar">
									<h4>Reporte: Certificados más vendidos:</h4>
								</div>
                ...
                <div class="col-sm-3 criterio_buscar">
                  <?php create_input('text','criterio_usu_per','',"form-control pull-right",$table,'placeholder="Ingresa ID certificado "'); ?>
                </div>
...
<script>
var link = "reportes_certificados_mas_vendido";/*la s final se agrega en js fuctions*/
var us = "reportes_certificados_mas_vendido";/*sirve para mensaje en ventana eliminar*/
var l = "o";
var l2 = "e";/* est+ _ x {e,a,o ,etc}sirve para mensaje en ventana eliminar*/
var pr = "El";
var ar = "al";
var id = "id_pedido";
var mypage = "reportes_certificados_mas_vendidos.php";
</script>
<?php } ?>