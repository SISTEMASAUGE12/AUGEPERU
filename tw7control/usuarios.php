...
if($_GET["task"]=='neworden'){
  ...
  $norden=_orden_noticia("","usuario","");
	if(empty($_POST['comision'])) $_POST['comision']='0'; 
  ...
  if(isset($_FILES['cumple_imagen']) && !empty($_FILES['cumple_imagen']['name'])){
    $_POST['cumple_imagen'] = carga_imagen('files/images/usuario/','cumple_imagen','','600','600');
  ...
  $campos=array('fecha_modificacion','idusuario_modifico',"estado_idestado",'nombre_corto','banco','cuenta_banco','idtipo_usu',"codusuario",'comision','toky','tipo_asesora',"nomusuario", "email",'cumple_detalle', array("contrasena",$contrasena));
...
<?php
  ...
  foreach ($_GET['order'] as $order => $item) {
    $orden = $orden + 1;
    $bd->actualiza_("UPDATE usuario SET orden= ".$orden." WHERE   idusuario = ".$item."");
  }
  $bd->Commit();
  $bd->close();
  ...
  if(isset($_GET['tipo_asesora']) && !empty($_GET['tipo_asesora'])){
		$sql.= " AND u.tipo_asesora='".$_GET['tipo_asesora']."' ";
	}
            ...
            <table id="example1" class="table table-bordered table-striped">
              <thead>
                <tr role="row">
                  ...
                  <th class="sort cnone"># preguntas_edit</th>
...
<?php while ($detalles = $paging->fetchResultado()): ?>
                <tr id="order_<?php echo $detalles["idusuario"]; ?>">
                  ...
                  <td><?php echo $detalles["idusuario"]; ?></td>
                  <!-- <td><?php echo $detalles["nomusuario"]; ?></td> -->
                  <td><?php echo $detalles["nombre_corto"]; ?></td>
                  <td class="cnone"><?php 
                      
                      $_sql= "select * from preguntas_bancos where idusuario=".$detalles["idusuario"]." ";
                    $total_preguntas=executesql($_sql);
                    echo "<p>".count($total_preguntas)."</p>";
...
-->
                ...
                <tr>
                  ...
                  <td><?php echo $detalles["fecha_modificacion"]; ?> </td>
                  <td></td>
                  <td></td>
                  <td></td>
                  <td></td>
            ...
            <form action="javascript: fn_buscar();" id="frm_buscar" name="frm_buscar">
              <div class="bg-gray-light">
                ...
                <div class="col-sm-1 criterio_buscar">
                  <select name="tipo_asesora" id="tipo_asesora" class="form-control" >
                      <option value="" > ver tipo_asesora</option>
