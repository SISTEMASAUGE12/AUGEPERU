...
if($_GET["task"]=='finder'){
  ...
  $sql= "SELECT sol.*,YEAR(sol.fecha_registro) as anho, MONTH(sol.fecha_registro) as mes, e.nombre AS estados, CONCAT(sol.nombre,' ',sol.ap_pa,' ',sol.ap_ma )as suscritos, dd.titulo as depa, pr.titulo as prov, dis.titulo as dist   
		FROM solicitudes_coautorias sol 
  	INNER JOIN dptos dd ON sol.iddpto=dd.iddpto
  	INNER JOIN prvc pr ON sol.idprvc=pr.idprvc
  	INNER JOIN dist dis ON sol.iddist=dis.iddist
  	INNER JOIN estado e ON sol.estado_idestado=e.idestado ";
  
  if (isset($_GET['criterio_mostrar'])) $porPagina=$_GET['criterio_mostrar'];
  if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per'])){
    $stringlike=fn_filtro(substr($_GET['criterio_usu_per'], 0, 16));
			$pos = strpos($sql, 'WHERE');
      $sql.= (($pos === false) ? "WHERE" : "AND" )." (sol.dni LIKE '".$stringlike."%' ) "; 
  }else{
		if( empty($_GET['fechabus_1']) && empty($_GET['fechabus_2']) ) {
			$pos = strpos($sql, 'WHERE');
      $sql.= (($pos === false) ? "WHERE" : "AND" )." DATE(sol.fecha_registro) < '" . fecha_hora(1) . "'";
		}
		
	}
	
	if(!empty($_GET['estado_pago']) ){
			$sql .= " AND sol.estado = '".$_GET['estado_pago']."'";
	}

		if(!empty($_GET['fechabus_1']) && !empty($_GET['fechabus_2'])) {
			$pos = strpos($sql, 'WHERE');
      $sql.= (($pos === false) ? "WHERE" : "AND" )." DATE(sol.fecha_registro)  BETWEEN  DATE('".$_GET['fechabus_1']."')  and DATE('".$_GET['fechabus_2']."')  ";		
	}
 ...
 $sql.= " ORDER BY sol.fecha_registro DESC";
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="reportes_solicitudes_coautorias.php";
?>
  <table id="example1" class="table table-bordered table-striped">
		...
		<a href="javascript:fn_exportar('tipo_form=1&fecha_filtro_inicio=<?php echo $fecha_excel_inicio;?>&fecha_filtro_fin=<?php echo $_GET["fechabus_2"];?>','solicitudes_coautorias');" class="btn btn-primary excel "  > Excel</a>
...
?>
          ...
          <th class="sort cnone">Departamento</th>
          <th class="sort cnone">Provincia</th>
          <th class="sort cnone">Distrito</th>
          <th class="sort cnone">Dirección</th>
...
?>        
       <tr>
        ...
        <td class="cnone"><?php echo $detalles["depa"]; ?></td>
        <td class="cnone"><?php echo $detalles["prov"]; ?></td>
        <td class="cnone"><?php echo $detalles["dist"]; ?></td>
        <td class="cnone"><?php echo $detalles["direccion"]; ?></td>
        <td class="cnone"><?php
        	if($detalles["estado"]==1){ echo 'Entregado';
        	}elseif($detalles["estado"]==2){ echo 'Pendiente';
        	}elseif($detalles["estado"]==3){ echo 'Rechazado';
        	}elseif($detalles["estado"]==4){ echo 'Procesando';
        	}elseif($detalles["estado"]==5){ echo 'Enviado';
        	}
       ?></td>
      </tr>
<?php endwhile; ?>
            ...
            <form action="javascript: fn_buscar();" id="frm_buscar" name="frm_buscar">
              <div class="bg-gray-light">
								<div class="col-sm-12 titulo_reporte criterio_buscar">
									<h4>Reporte: Solicitudes de CoAutorías:</h4>
								</div>
...
<script>
var link = "reportes_solicitudes_coautoria";/*la s final se agrega en js fuctions*/
var us = "reportes_solicitudes_coautoria";/*sirve para mensaje en ventana eliminar*/
var l = "o";
var l2 = "e";/* est+ _ x {e,a,o ,etc}sirve para mensaje en ventana eliminar*/
var pr = "El";
var ar = "al";
var id = "id_pedido";
var mypage = "reportes_solicitudes_coautorias.php";
</script>
<?php } ?>