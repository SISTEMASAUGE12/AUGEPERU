...
if($_GET["task"]=='neworden'){
  ...
  $bd=new BD;
	...
	if(!empty($_POST["hora_inicio_lunes"])){
		$campos=array_merge($campos, array('hora_inicio_lunes'));
	}
	if(!empty($_POST["hora_fin_lunes"])){
		$campos=array_merge($campos, array('hora_fin_lunes'));
	}
	if(!empty($_POST["hora_inicio_martes"])){
		$campos=array_merge($campos, array('hora_inicio_martes'));
	}
	if(!empty($_POST["hora_fin_martes"])){
		$campos=array_merge($campos, array('hora_fin_martes'));
	}
	if(!empty($_POST["hora_inicio_miercoles"])){
		$campos=array_merge($campos, array('hora_inicio_miercoles'));
	}
	if(!empty($_POST["hora_fin_miercoles"])){
		$campos=array_merge($campos, array('hora_fin_miercoles'));
	}
	if(!empty($_POST["hora_inicio_jueves"])){
		$campos=array_merge($campos, array('hora_inicio_jueves'));
	}
	if(!empty($_POST["hora_fin_jueves"])){
		$campos=array_merge($campos, array('hora_fin_jueves'));
	}
	if(!empty($_POST["hora_inicio_viernes"])){
		$campos=array_merge($campos, array('hora_inicio_viernes'));
	}
	if(!empty($_POST["hora_fin_viernes"])){
		$campos=array_merge($campos, array('hora_fin_viernes'));
	}
	if(!empty($_POST["hora_inicio_sabado"])){
		$campos=array_merge($campos, array('hora_inicio_sabado'));
	}
	if(!empty($_POST["hora_fin_sabado"])){
		$campos=array_merge($campos, array('hora_fin_sabado'));
	}
	if(!empty($_POST["hora_inicio_domingo"])){
		$campos=array_merge($campos, array('hora_inicio_domingo'));
	}
	if(!empty($_POST["hora_fin_domingo"])){
		$campos=array_merge($campos, array('hora_fin_domingo'));
	}
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"cursos","id_curso","titulo_rewrite",$where);
	...
	// if(empty($_POST['validez_meses'])) $_POST['validez_meses']='0'; // esta autamtico a solo 6m 
	...
	$campos=array('tag','tag_trafico','id_tipo','titulo_trafico','id_tipo_curso','tipo','titulo', array('titulo_rewrite',$urlrewrite),"breve_detalle","precio",'modalidad',"link_video",'descripcion','estado_idestado','fecha_actualizacion','visibilidad','en_vivo_finalizado','etiqueta_fondo','etiqueta','titulo_banner_detalle','titulo_boton'); 
  ...
  if($_GET["task"]=='insert'){
	...
	if(isset($_FILES['imagen_mini']) && !empty($_FILES['imagen_mini']['name'])){
		$_POST['imagen_mini'] = carga_imagen('files/images/capa/','imagen_mini','','100','100');
		$campos = array_merge($campos,array('imagen_mini'));
	  }
	
	  if(isset($_FILES['imagen_4']) && !empty($_FILES['imagen_4']['name'])){
		$_POST['imagen_4'] = carga_imagen('files/images/capa/','imagen_4','','802','506');
		$campos = array_merge($campos,array('imagen_4'));
	  }
	
	  if(isset($_FILES['archivo']) && !empty($_FILES['archivo']['name'])){
		$_POST['archivo'] = carga_imagen('files/images/capa/','archivo','','100','100');
...
IF($ultima_venta<10){
  ...
  $_POST["codigo"]= "CU".$ultima_venta;
	...
	if(isset($_FILES['imagen_mini']) && !empty($_FILES['imagen_mini']['name'])){
		$path = 'files/images/capa/'.$_POST['imagen_mini_ant'];
		if( file_exists($path) && !empty($_POST['imagen_mini_ant']) ) unlink($path);    
		$_POST['imagen_mini'] = carga_imagen('files/images/capa/','imagen_mini','','813','450');
		$campos = array_merge($campos,array('imagen_mini'));
	  }
	
	  if(isset($_FILES['imagen_4']) && !empty($_FILES['imagen_4']['name'])){
		$path = 'files/images/capa/'.$_POST['imagen_4_ant'];
		if( file_exists($path) && !empty($_POST['imagen_4_ant']) ) unlink($path);    
		$_POST['imagen_4'] = carga_imagen('files/images/capa/','imagen_4','','802','506');
		$campos = array_merge($campos,array('imagen_4'));
	  }
	
	  if(isset($_FILES['archivo']) && !empty($_FILES['archivo']['name'])){
		$path = 'files/images/capa/'.$_POST['archivo_ant'];
		if( file_exists($path) && !empty($_POST['archivo_ant']) ) unlink($path);    
		$_POST['archivo'] = carga_imagen('files/images/capa/','archivo','','813','450');
		$campos = array_merge($campos,array('archivo'));
	  }
    ...
    $bd->actualiza_(armaupdate('cursos',array_merge($campos,array('idusuario_modifico','fecha_modifico')) ," id_curso='".$_POST["id_curso"]."'",'POST'));/*actualizo*/
  }
...
*/ ?>
				...
				<div class="form-group">
                  <label for="inputPassword3" class="col-md-2 col-sm-2 control-label">Título banner *detalle</label>
                  <div class="col-sm-6">
                    <?php create_input("text","titulo_banner_detalle",$data_producto["titulo_banner_detalle"],"form-control",$table,"",$agregado); ?>
                  </div>
                </div>
				<div class="form-group">
                  <label for="inputPassword3" class="col-md-2 col-sm-2 control-label">Título del boton</label>
                  <div class="col-sm-6">
				...
				<div class="form-group">
                  ...
                  <div class="col-sm-6">
							...
							<div class="form-group   "> <!-- oculto momentaneatee -->
								<label for="inputPassword3" class="col-md-2 col-sm-2 control-label"><b style="color:red;">Precio ONLINE S/.  [ promo landing ]</b> </label>
								<div class="col-md-2 col-sm-2">
							...
							<div class="form-group">
								<label for="inputPassword3" class="col-md-2 col-sm-2 control-label">
									Imágen - listado de cursos<small>(*listado)</small><br/>
									<span class="red">349px ancho * 276px alto</span></label>
									<div class="col-sm-6">
				...
				<div class="form-group">
				  <label for="inputPassword3" class="col-md-2 col-sm-2 control-label">
										Imágen Grande: detalle_curso pagina [*nuevo]<br/>
										<span class="red">802px ancho * 506px alto</span></label>
				  <div class="col-sm-6">
					<input type="file" name="imagen_4" id="imagen_4" class="form-control">
					<?php create_input("hidden","imagen_4_ant",$data_producto["imagen_4"],"",$table,$agregado); 
					  if($data_producto["imagen_4"]!=""){ 
					?>
					  <img src="<?php echo "files/images/capa/".$data_producto["imagen_4"]; ?>" width="200" class="mgt15">
					<?php } ?> 
				  </div>
				</div>
				...
				<div class="form-group">
				  <label for="inputPassword3" class="col-md-2 col-sm-2 control-label">
										Imágen Grande: detalle_curso flotante<br/>
										<span class="red">650px ancho * 300px alto</span></label>
				  <div class="col-sm-6">
				...
				<div class="form-group">
					<label for="inputPassword3" class="col-md-2 col-sm-2 control-label">
						Imágen mini - mis cursos<small>(*comprados)</small><br/>
						<span class="red">100px ancho * 100px alto</span></label>
						<div class="col-sm-6">
							<input type="file" name="imagen_mini" id="imagen_mini" class="form-control">
							<?php create_input("hidden","imagen_mini_ant",$data_producto["imagen_mini"],"",$table,$agregado); 
                      if($data_producto["imagen_mini"]!=""){ 
						  ?>
                      <img src="<?php echo "files/images/capa/".$data_producto["imagen_mini"]; ?>" width="200" class="mgt15">
					  <?php } ?> 
					</div>
                </div>
				...
				<div class="form-group">
				  <label for="inputPassword3" class="col-md-2 col-sm-2 control-label">Malla curricular PDF<small>(*brochure)</small><br/></label>
				  <div class="col-sm-6">
					<input type="file" name="archivo" id="archivo" class="form-control">
					<?php create_input("hidden","archivo_ant",$data_producto["archivo"],"",$table,$agregado); 
					  if($data_producto["archivo"]!=""){ 
					?>
					  <img src="<?php echo "files/images/capa/".$data_producto["archivo"]; ?>" width="200" class="mgt15">
					<?php } ?> 
				  </div>
				</div>
								
				<div class="form-group">
                  <label for="inputPassword3" class="col-md-2 col-sm-2 control-label">Link video trailer VIMEO </label>
                  <div class="col-sm-6">
					<p> ejem: https://player.vimeo.com/video/902085563?h=8460def682&badge=0&autopause=0&player_id=0&app_id=58479 </p>
                    <?php create_input("text","link_video",$data_producto["link_video"],"form-control lleva_link_vimeo ",$table,"",$agregado); ?>
				...
				<div class="form-group">
                  <label for="inputPassword3" class="col-sm-2 control-label">Dirigido a: </label>
                  <div class="col-sm-10">
                    <?php create_input("text","inicio",$data_producto["inicio"],'  form-control  ',$table,'style="height: !important;"');  ?>
                    <script>
				...
				<div class="form-group">
                  ...
                  <div class="col-md-10 ">
                    <?php create_input("text","frecuencia",$data_producto["frecuencia"],' form-control ',$table,'style="height: !important;"');  ?>
                    <script>
				...
				<div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <?php create_input("text","horario",$data_producto["horario"],' form-control ',$table,'style="height: !important;"');  ?>
                    <script>
				...
				<div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <?php create_input("text","modalidad_texto",$data_producto["modalidad_texto"],' form-control  ',$table,'style="height: !important;"');  ?>
                    <script>
				...
				<div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <?php create_input("text","duracion",$data_producto["duracion"],'  form-control  ',$table,'style="height: !important;"');  ?>
                    <script>
				...
				*/ ?>				
				...
				<div class="form-group">
                  <label for="inputPassword3" class="col-md-2 col-sm-2 control-label">Breve intro:</label>
                  <div class="col-sm-6">
...
<?php
  ...
  if(isset($_GET['criterio_ordenar_por'])) $sql.= sprintf(" order by %s %s", fn_filtro($_GET['criterio_ordenar_por']), fn_filtro($_GET['criterio_orden']));
	...
	$paging->verPost(true);
	$mantenerVar=array("id_curso",'id_tipo',"criterio_mostrar","task","criterio_usu_per","criterio_ordenar_por","criterio_orden"); // agregr los post s ia lurl actual los neceista 
...
<?php }//if meses ?>
                  ...
                  <td>
										...
										<a href="https://www.educaauge.com/unete/<?php echo $detalles["titulo_rewrite"]; ?>" target="_blank">[link trafico]</a> </br>
										<a href="https://www.educaauge.com/cesta-pago-tarjeta-directo/<?php echo $detalles["id_curso"]; ?>" target="_blank">[link pago_directo]</a>
									</td>                                
                  ...
                  <td class=" cnone ">
                    <div class="btn-eai  text-center btns btr  ">				
									...
									<td class=" cnone ">
											<a href="index.php?page=silabos&id_curso=<?php echo $detalles['id_curso']; ?>&module=<?php echo $_GET['module']; ?>&parenttab=<?php echo $_GET['parenttab']; ?>"  style="background:blue;padding:6px;color:#fff;border-radius:8px;" title="Agregar silabos"><span> temarios</span>
											</a> 
									</td>
                    ...
                    <div class="btn-eai  text-center btns btr  ">				
									...
									<?php }else{ echo "--"; } ?>
                  					...
                  					<td class=" cnone ">
										<span style="color:#000;"><?php echo $detalles["usuario"]; ?></span>
										<br>
										Modificado:<?php echo ($_SESSION["visualiza"]["idusuario"] == $detalles["idusuario_modifico"])?ucwords($_SESSION['visualiza']['nomusuario']):$detalles["idusuario_modifico"]; ?>
										<br>
										Fecha modifico: <?php echo $detalles["fecha_modifico"]; ?> 
									</td>
									
									<td>
                    <div class="btn-eai  text-center btns btr   btn_varios ">				
											<a href="index.php?page=cursos&module=Ver&parenttab=cursos&id_tipo=<?php echo $_GET["id_tipo"].'&task=edit&id_curso='.$detalles["id_curso"]; ?>" style="color:#fff;"><i class="fa fa-edit"></i> <span>edit</span>
											</a>
				...
				<div class="modal-content text-center">
					<div class="modal-header">
						...
						<div class="form-group data_curso_en_vivo text-left " style=" max-width:800px;background: #efefef;padding: 20px;border-radius: 6px;margin-bottom:25px;">
							...
							<label for="inputPassword3" class="col-md-12 control-label" style="color:green;">Solo admin y Valeria tiene acceso a esta opción </label>
					
					<?php if( $_SESSION["visualiza"]["idtipo_usu"] == 1  || $_SESSION["visualiza"]["idusuario"] == 82 ){  // Solo admin y Valeria tiene acceso a esta opción  ?>
							<div class="col-sm-12">
								<?php create_input("text","enlace_en_vivo_".$detalles['id_curso'],$detalles["enlace_en_vivo"],"form-control",$table,"  ",$agregado); ?>
							</div>
					<?php } ?> 						

							<label for="inputPassword3" class="col-md-12  control-label">Hora en vivo: [boton verde ] * textual </label>
							<div class="col-sm-12">
								<?php create_input("text","hora_en_vivo_".$detalles['id_curso'],$detalles["hora_en_vivo"],"form-control",$table,"  ",$agregado); ?>
							</div>
							
							<div class="col-sm-12 " style="padding:9px; ">
								<h4 style=""><b>DATA algoritmo: formato 24horas  (ejemplo: 15 => 3pm) </b></h4>
								<label for="inputPassword3" class="col-md-12  control-label" style="color:red;">* considera unos 20min antes del inicio real</label>
							</div>


							<!-- LUNES -->
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora inicio: * Lunes </label>
								<?php 
								$name_hora_inicio_lunes= "hora_inicio_lunes_".$detalles['id_curso'];
								$name_hora_fin_lunes= "hora_fin_lunes_".$detalles['id_curso'];   
								?>
								<input type="time" name="<?php echo $name_hora_inicio_lunes; ?>" id="<?php echo $name_hora_inicio_lunes; ?>" value="<?php echo  !empty($detalles["hora_inicio_lunes"])?$detalles["hora_inicio_lunes"]:'';?>">
							</div>
							
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora fin: * Lunes </label>
								<div class="col-sm-12">
									<input type="time" name="<?php echo $name_hora_fin_lunes; ?>" id="<?php echo $name_hora_fin_lunes; ?>" value="<?php echo  !empty($detalles["hora_fin_lunes"])?$detalles["hora_fin_lunes"]:'';?>">
								</div>
							</div>
							<!-- END LUNES -->
							...
							<!-- martes  -->
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora inicio: * martes </label>
								<?php 
								$name_hora_inicio_martes= "hora_inicio_martes_".$detalles['id_curso'];
								$name_hora_fin_martes= "hora_fin_martes_".$detalles['id_curso'];   
								?>
								<input type="time" name="<?php echo $name_hora_inicio_martes; ?>" id="<?php echo $name_hora_inicio_martes; ?>" value="<?php echo  !empty($detalles["hora_inicio_martes"])?$detalles["hora_inicio_martes"]:'';?>">
							</div>
							
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora fin: * martes </label>
								<div class="col-sm-12">
									<input type="time" name="<?php echo $name_hora_fin_martes; ?>" id="<?php echo $name_hora_fin_martes; ?>" value="<?php echo  !empty($detalles["hora_fin_martes"])?$detalles["hora_fin_martes"]:'';?>">
								</div>
							</div>
							<!-- END martes  -->
							...
							<!-- miercoles  -->
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora inicio: * Miercoles </label>
								<?php 
								$name_hora_inicio_miercoles= "hora_inicio_miercoles_".$detalles['id_curso'];
								$name_hora_fin_miercoles= "hora_fin_miercoles_".$detalles['id_curso'];   
								?>
								<input type="time" name="<?php echo $name_hora_inicio_miercoles; ?>" id="<?php echo $name_hora_inicio_miercoles; ?>" value="<?php echo  !empty($detalles["hora_inicio_miercoles"])?$detalles["hora_inicio_miercoles"]:'';?>">
							</div>
							
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora fin: * Miercoles </label>
								<div class="col-sm-12">
									<input type="time" name="<?php echo $name_hora_fin_miercoles; ?>" id="<?php echo $name_hora_fin_miercoles; ?>" value="<?php echo  !empty($detalles["hora_fin_miercoles"])?$detalles["hora_fin_miercoles"]:'';?>">
								</div>
							</div>
							<!-- END miercoles -->
							...
							<!-- jueves  -->
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora inicio: * Jueves </label>
								<?php 
								$name_hora_inicio_jueves= "hora_inicio_jueves_".$detalles['id_curso'];
								$name_hora_fin_jueves= "hora_fin_jueves_".$detalles['id_curso'];   
								?>
								<input type="time" name="<?php echo $name_hora_inicio_jueves; ?>" id="<?php echo $name_hora_inicio_jueves; ?>" value="<?php echo  !empty($detalles["hora_inicio_jueves"])?$detalles["hora_inicio_jueves"]:'';?>">
							</div>
							
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora fin: * Jueves </label>
								<div class="col-sm-12">
									<input type="time" name="<?php echo $name_hora_fin_jueves; ?>" id="<?php echo $name_hora_fin_jueves; ?>" value="<?php echo  !empty($detalles["hora_fin_jueves"])?$detalles["hora_fin_jueves"]:'';?>">
								</div>
							</div>
							<!-- END jueves  -->
							...
							<!-- viernes  -->
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora inicio: * Viernes </label>
								<?php 
								$name_hora_inicio_viernes= "hora_inicio_viernes_".$detalles['id_curso'];
								$name_hora_fin_viernes= "hora_fin_viernes_".$detalles['id_curso'];   
								?>
								<input type="time" name="<?php echo $name_hora_inicio_viernes; ?>" id="<?php echo $name_hora_inicio_viernes; ?>" value="<?php echo  !empty($detalles["hora_inicio_viernes"])?$detalles["hora_inicio_viernes"]:'';?>">
							</div>
							
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora fin: * Viernes </label>
								<div class="col-sm-12">
									<input type="time" name="<?php echo $name_hora_fin_viernes; ?>" id="<?php echo $name_hora_fin_viernes; ?>" value="<?php echo  !empty($detalles["hora_fin_viernes"])?$detalles["hora_fin_viernes"]:'';?>">
								</div>
							</div>
							<!-- END vierens   -->
							...
							<!-- sabado  -->
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora inicio: * Sabado </label>
								<?php 
								$name_hora_inicio_sabado= "hora_inicio_sabado_".$detalles['id_curso'];
								$name_hora_fin_sabado= "hora_fin_sabado_".$detalles['id_curso'];   
								?>
								<input type="time" name="<?php echo $name_hora_inicio_sabado; ?>" id="<?php echo $name_hora_inicio_sabado; ?>" value="<?php echo  !empty($detalles["hora_inicio_sabado"])?$detalles["hora_inicio_sabado"]:'';?>">
							</div>
							
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora fin: * Sabado </label>
								<div class="col-sm-12">
									<input type="time" name="<?php echo $name_hora_fin_sabado; ?>" id="<?php echo $name_hora_fin_sabado; ?>" value="<?php echo  !empty($detalles["hora_fin_sabado"])?$detalles["hora_fin_sabado"]:'';?>">
								</div>
							</div>
							<!-- END sabado   -->
							...
							<!-- domingo  -->
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora inicio: * Domingo </label>
								<?php 
								$name_hora_inicio_domingo= "hora_inicio_domingo_".$detalles['id_curso'];
								$name_hora_fin_domingo= "hora_fin_domingo_".$detalles['id_curso'];   
								?>
								<input type="time" name="<?php echo $name_hora_inicio_domingo; ?>" id="<?php echo $name_hora_inicio_domingo; ?>" value="<?php echo  !empty($detalles["hora_inicio_domingo"])?$detalles["hora_inicio_domingo"]:'';?>">
							</div>
							
							<div class="col-sm-6 ">
								<label for="inputPassword3" class="col-md-12  control-label">Hora fin: * Domingo </label>
								<div class="col-sm-12">
									<input type="time" name="<?php echo $name_hora_fin_domingo; ?>" id="<?php echo $name_hora_fin_domingo; ?>" value="<?php echo  !empty($detalles["hora_fin_domingo"])?$detalles["hora_fin_domingo"]:'';?>">
								</div>
							</div>
							<!-- END domingo    -->
							...
							<div class="col-sm-12 " style="padding:9px; ">
								...
								<label for="inputPassword3" class="col-md-12  control-label" style="color:red;">* Debes completar la información en los días que se va llevar una clase en vivo, ejemplo: si los viernes hay clases el campo del formulario <b>Viernes: ejem[mate5]</b>  debe tener un contenido para que el algoritmo valide que ese día hay clases en la hora establecida en los campos anteriores    </label>
...
?>						
	...
	$(function(){
		checked();
		sorter();
		// reordenar('cursos.php'); // antiguo colapsa con ardata data 
		// reordenar_con_post('cursos.php');  // moderno todo okey pero suspendidor porq demora en cargar el listado total 
