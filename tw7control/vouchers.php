...
if($_GET["task"]=='drop' || $_GET["task"]=='dropselect'){
  ...
  $sql= "SELECT v.*,YEAR(v.fecha_registro) as anho, MONTH(v.fecha_registro) as mes, CONCAT(s.nombre,' ',s.ap_pa,' ',s.ap_ma )as suscritos, bb.nombre as banco, pp.codigo, s.dni, pp.estado_pago, pp.tipo_pago 
		FROM vouchers v 
 ...
 //  echo $sql; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="vouchers.php";
?>
...
?>
				...
				<tr role="row">
					<th width="30">Día</th>
          <th class="sort">TIPO_VENTA</th>
          <th class="sort">Banco</th>
          <th class="sort">Código Operación</th>
          <th class="sort cnone" >Código Pedido</th>
          <th class="sort cnone">DNI</th>
          <th class="sort cnone">Cliente</th>
          <th class="sort ">Estado envío</th>
          <th class="sort ">LIBERAR</th>
         </tr>
<?php }//if meses 
...
?>        
       ...
       ?></td>
        ...
        <td class="cnone"><?php echo $detalles["codigo_operacion"]; ?></td>        
       ...
       ?></td>
        <td class="cnone">
					<div class="btn-eai btns text-center btr">
						<a href="javascript: fn_eliminar('<?php echo $detalles["id_vouchers"]; ?>')" style="color:#fff;background:red;" title="Eliminar"><i class="fa fa-trash-o"></i></a>
					</div>    
				</td>    
      </tr>
...
<script>
var link = "voucher";/*la s final se agrega en js fuctions*/
