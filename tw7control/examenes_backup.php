...
if($_GET["task"]=='neworden'){
  ...
  $campos=array('id_cate','id_especialidad','privacidad','titulo', array('titulo_rewrite',$urlrewrite),'minutos','total_preguntas','cant_intentos','estado_examen','fecha_cierre','fecha_actualizacion','estado_idestado'); 
	
  if($_GET["task"]=='insert'){
    if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $_POST['imagen'] = carga_imagen('files/images/examenes/','imagen','');
    ...
    $_POST['fecha_registro'] = fecha_hora(2);
		...
		// echo var_dump(arma_insert('examenes',array_merge($campos,array('fecha_registro','orden')),'POST'));
		// exit();
...
<?php 
  ...
  if(!empty($examenes)){
    foreach($examenes as $row){
      $pfile = 'files/images/imagenes/'.$row['imagen'];  if(file_exists($pfile) && !empty($row['imagen'])){ unlink($pfile); }
      $pfile = 'files/images/imagenes/'.$row['imagen2'];  if(file_exists($pfile) && !empty($row['imagen2'])){ unlink($pfile); }
      $pfile = 'files/images/imagenes/'.$row['imagen_pre_2'];  if(file_exists($pfile) && !empty($row['imagen_pre_2'])){ unlink($pfile); }
          ...
          <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
            <form action="javascript: fn_buscar();" id="frm_buscar" name="frm_buscar">
              ...
              <div class="bg-gray-light">
                ...
                <div class="col-md-2">
                  <div class="btn-eai">
                    <a href="<?php echo $link_examen_aleatorio."&task=new"; ?>" style="color:#fff;background:green;"><i class="fa fa-file"></i> Crear aleatorio</a>                    
                  </div>
                </div>
