...
var us = "reportes_compras_clientes_ventas";/*sirve para mensaje en ventana eliminar*/
var l = "o";
var l2 = "e";/* est+ _ x {e,a,o ,etc}sirve para mensaje en ventana eliminar*/
var pr = "El";
var ar = "al";
var id = "id_pedido";
var mypage = "reportes_compras_clientes_ventas.php";
</script>
</section><!-- /.content -->
<?php
  ...
  $sql= "SELECT pp.*,YEAR(pp.fecha_registro) as anho, MONTH(pp.fecha_registro) as mes, e.nombre AS estado ,s.email as email,  CONCAT(s.nombre,' ',s.ap_pa,' ',s.ap_ma )as suscritos , s.dni as dni  , ban.nombre as banco,  u.nomusuario as usuariox   
	FROM pedidos pp 
  ...
  if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per'])){
    ...
    $pos = strpos($sql, 'WHERE');
      $sql.= (($pos === false) ? "WHERE" : "AND" )."  ( s.dni LIKE '".$stringlike."%' or s.telefono LIKE '".$stringlike."%' or  s.nombre LIKE '".$stringlike."%' or s.email LIKE '".$stringlike."%' or pp.codigo LIKE '".$stringlike."%'  or pp.id_pedido LIKE '".$stringlike."%' ) "; 
  }else{
 ...
 $sql.= " ORDER BY pp.id_pedido DESC";
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="reportes_compras_clientes_ventas.php";
?>
...
?>
      <tr>
      	...
      	<td><b>Curso: </b><?php echo $linea['titulo'] ?></td>
...
<script>
var link = "reportes_compras_clientes_venta";/*la s final se agrega en js fuctions*/
