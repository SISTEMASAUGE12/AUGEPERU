...
if($_POST["task"]=='marcar_enviado'){
  ...
  $campos=array('estado','empresa_envio',"comentario"); 
	...
	// echo var_dump(armaupdate('solicitudes_coautorias',$campos," ide='".$_POST["ide"]."'",'POST'));
	// exit(); 
	
	$bd->actualiza_(armaupdate('solicitudes_coautorias',$campos," ide='".$_POST["ide"]."'",'POST'));/*actualizo*/
  $bd->close();
  ...
  if($_GET["task"]=='edit'){
		$sql_xx="select sau.*, esp.titulo as especialidad from solicitudes_coautorias sau  INNER JOIN especialidades esp ON sau.id_especialidad=esp.id_especialidad where sau.ide='".$_GET["ide"]."'";
		
     $usuario=executesql($sql_xx,0);
  } 
?>
    ...
    <div class="col-md-12">
          ...
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">Solicitudes coautoria de Libro</h3>
            </div><!-- /.box-header -->
<?php $task_=$_GET["task"]; ?>
            <!-- form start -->
                    ...
                    <div id="collapseOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne">
                      <div class="panel-body">
                        ...
                        <table  class="table table-bordered table-striped">
                          <thead>
                            <tr role="row">
                              <th class="sort cnone" width="50">CURSO</th>
                    ...
                    <div id="collapseThree" class="panel-collapse collapse in " role="tabpanel" aria-labelledby="headingThree">
                      <div class="panel-body">
                          ...
                          <?php create_input("text","email",$usuario["email"],"form-control",$table,"disabled",$agregado); ?>
												...
												<div class="form-group">
													...
													<div class="col-sm-4 criterio_buscar">
														<?php crearselect("iddpto", "select iddpto,titulo from dptos order by titulo asc", 'class="form-control"  onchange="javascript:display(\'solicitudes_coautorias.php\',this.value,\'cargar_prov2\',\'idprvc\')"', $usuario["iddpto"], "-- Dptos --"); ?>
													</div>
												</div>
												...
												<div class="form-group">
														...
														<div class="col-sm-4 criterio_buscar">
																	<!--
															 crearselect("idprvc","select idprvc,titulo from prvc WHERE iddpto='".$usuario["iddpto"]."'",'class="form-control"  onchange="javascript:display(\'solicitudes_coautorias.php\',this.value,\'cargar_dist2\',\'iddist\')"',$usuario["idprvc"],"-- Prov --");
																			-->
																	<?php if ($task_ == 'edit') {
																			$sql = "select idprvc,titulo from prvc WHERE dptos_iddpto='" . $usuario["iddpto"] . "' "; ?>
																			<select name="idprvc" id="idprvc" class="form-control"
																							onchange="javascript:display('solicitudes_coautorias.php',this.value,'cargar_dist2','iddist')">
																					...
																					foreach ($listaprov as $data) { ?>
																							<option value="<?php echo $data['idprvc']; ?>"
																											selected="<?php echo ($data['idprvc'] == $usuario["idprvc"]) ? 'selected' : ''; ?>"> <?php echo $data['titulo'] ?></option>
																					<?php } ?>
																			</select>
																	<?php } else { ?>
																			<select name="idprvc" id="idprvc" class="form-control"
																	...
																	<select name="iddist" id="iddist" class="form-control">
																			...
																			foreach ($listaprov as $data) { ?>
																					<option value="<?php echo $data['iddist']; ?>"
																									selected="<?php echo ($data['iddist'] == $usuario["iddist"]) ? 'selected' : ''; ?>"> <?php echo $data['titulo'] ?></option>
																			<?php } ?>
																	</select>
															<?php } else { ?>
																	<select name="iddist" id="iddist" class="form-control">
                          ...
                          <?php create_input("text","direccion",$usuario["direccion"],"form-control",$table,"disabled",$agregado); ?>
												...
												<div class="form-group">
                          <label for="inputPassword3" class="col-sm-2 control-label"><b>ESPECIALIDAD :</b></label>
                          <div class="col-sm-3">
                          <?php create_input("text","especialidad",$usuario["especialidad"],"form-control",$table,"disabled",$agregado); ?>
                          </div>
                        </div>  
												
												<div class="form-group">
                          <label for="inputPassword3" class="col-sm-2 control-label">FECHA NAC:</label>
                          <div class="col-sm-3">
                          <?php create_input("text","fecha_nac",$usuario["fecha_nac"],"form-control",$table,"disabled",$agregado); ?>
                          </div>
                        </div>  
												
												<div class="form-group">
                          <label for="inputPassword3" class="col-sm-2 control-label">DIRECCIÓN NACIMIENTO:</label>
                          <div class="col-sm-3">
                          <?php create_input("text","direccion_nac",$usuario["direccion_nac"],"form-control",$table,"disabled",$agregado); ?>
                          </div>
                        </div>  
...
var ar = "la";
var id = "ide";
var mypage = "solicitudes_coautorias.php";
</script>
</section><!-- /.content -->
<?php
  ...
  $usuario = executesql("SELECT * FROM solicitudes_coautorias WHERE ide IN (".$ide.")");
  if(!empty($usuario))
  ...
  $num_afect=$bd->actualiza_("UPDATE solicitudes_coautorias SET estado=".$state." WHERE ide=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
  ...
  // $sql= "SELECT pp.*,YEAR(pp.fecha_registro) as anho, MONTH(pp.fecha_registro) as mes, s.email as email,  CONCAT(s.nombre,' ',s.ap_pa,' ',s.ap_ma )as suscritos, cer.titulo as certificado, cer.id_certificado as cod_certi, c.titulo as curso, c.codigo as cod_curso  
	// FROM solicitudes_coautorias pp 
  ...
  // INNER JOIN suscritos s ON pp.id_suscrito=s.id_suscrito 
	 ...
	 // $sql= "SELECT pp.*,YEAR(pp.fecha_registro) as anho, MONTH(pp.fecha_registro) as mes, s.email as email_cliente,  CONCAT(s.nombre,' ',s.ap_pa,' ',s.ap_ma )as suscritos, esp.titulo as especialidad 
	// FROM solicitudes_coautorias pp 
  // INNER JOIN especialidades esp ON pp.id_especialidad=esp.id_especialidad   
  // LEFT JOIN suscritos s ON pp.id_suscrito=s.id_suscrito 
	// WHERE  pp.id_especialidad=esp.id_especialidad    
	 // "; 

	 $sql= "SELECT pp.*,YEAR(pp.fecha_registro) as anho, MONTH(pp.fecha_registro) as mes,  esp.titulo as especialidad 
	FROM solicitudes_coautorias pp 
  INNER JOIN especialidades esp ON pp.id_especialidad=esp.id_especialidad   
	WHERE     pp.id_especialidad=esp.id_especialidad  
	 "; 
  ...
  if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per'])){
    ...
    $sql.= " AND ( pp.dni LIKE '".$stringlike."%' or pp.nombre LIKE '".$stringlike."%' or pp.dni LIKE '".$stringlike."%' or pp.email LIKE '".$stringlike."%' or pp.telefono LIKE '".$stringlike."%' or pp.nombre_magisterio LIKE '".$stringlike."%' or pp.ap_pa LIKE '".$stringlike."%' or pp.ap_ma LIKE '".$stringlike."%'  )"; 
  }else{
 ...
 // echo  $sql; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="solicitudes_coautorias.php";
?>
  <table id="example1" class="table table-bordered table-striped">
		...
		<a href="javascript:fn_exportar('tipo_form=1&fecha_filtro_inicio=<?php echo $fecha_excel_inicio;?>&fecha_filtro_fin=<?php echo $_GET["fechabus_2"];?>','coautorias');" class="btn btn-primary"  > Excel</a>
...
?>
          ...
          <th class="sort cnone" >NOMBRE MAGISTERIO</th>
...
<script>
$(function(){
  // reordenar('solicitudes_coautorias.php');
  // checked();
  // sorter();
});
</script>
