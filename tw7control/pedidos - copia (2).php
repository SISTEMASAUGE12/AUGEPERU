		...
		if( !empty($_GET['id_pedido'] ) && $_GET['id_pedido'] > 0 ){ // si existe id_pedido hacemos el recorrido 
			...
			// if(!empty($linea_pedido) ){ 
				// foreach($linea_pedido as $data ){
						// $bd->actualiza_(armaupdate('suscritos_x_cursos',$campos," id_pedido='".$_GET["id_pedido"]."'",'POST'));
			
				// } // for
			// } // if
			$bd->actualiza_(armaupdate('suscritos_x_cursos',$campos," id_pedido='".$_GET["id_pedido"]."'",'POST'));
			$bd->actualiza_(armaupdate('suscritos_x_certificados',$campos," id_pedido='".$_GET["id_pedido"]."'",'POST'));
...
?>        
       <tr >
        ...
        <td class="cnone"><?php echo $detalles["fecha_pago_off"]; ?></td>   
				...
				<?php if(!empty($_GET['tipo_pago']) && $_GET['tipo_pago']==3 ){ ?>
        <td > <?php echo $detalles["comentario"]; ?></td>  <!-- coemntario status de pago efectivo -->
				<?php } ?>
