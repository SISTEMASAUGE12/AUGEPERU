...
$nombre = $_POST['nombrebusqueda'];//recibe el nombre a buscar , solo cursos especialidades 

$sql="SELECT c.*, tp.titulo as tipo_curso FROM cursos c INNER JOIN tipo_cursos tp ON c.id_tipo=tp.id_tipo WHERE  c.id_tipo=1 and id_tipo_curso=2 and (c.codigo LIKE '%".$nombre."%' or c.titulo LIKE '%".$nombre."%' ) ORDER BY c.titulo ASC";
