...
// ESPECIALIDADES Y DEPENDIENTES 	
		$especialidad=executesql(" select * from suscritos where id_suscrito='".$_POST["id_suscrito"]."' ");
		...
		if(!empty($curso)){
			foreach($curso as $row){
				...
				/* CURSOS ESPECIALIDADES  .. */
						if( !empty($row['cursos_especialidades']) && !empty($especialidad[0]["id_especialidad"]) ){
							
							$especialidad_del_cliente=$especialidad[0]["id_especialidad"];
								...
								foreach( $especialidades as $anexo ){
										...
										if(!empty($validate_curso_existente_especialidades)){ 
											...
											// ASIGNAMOS CURSOS especialidades ..	
													...
													$_POST['orden'] = 1;
													$campos=array('id_suscrito',array('id_curso',$anexo["id_curso"]),'id_pedido','dependiente','especialidades','orden','fecha_registro',array('validez_meses',$validez_meses_dependientes),'estado','estado_idestado');
													
													if( isset($_SESSION["visualiza"]["idusuario"]) ){
												...
												/* --- */
													...
													/* si el curso Dependiente tiene especialidades, tbm agregamos las especialidad */
														/* CURSOS ESPECIALIDADES  de DEPEMDIENTES .. */
																if( !empty($anexo['cursos_especialidades']) && !empty($especialidad[0]["id_especialidad"]) ){
