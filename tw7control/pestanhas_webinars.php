...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"pestanhas_webinars","id_pesta","titulo_rewrite",$where);
  ...
  if($_GET["task"]=='insert'){
    ...
    $_POST['orden'] = _orden_noticia("","pestanhas_webinars","");
    $bd->inserta_(arma_insert('pestanhas_webinars',array_merge($campos,array('orden')),'POST'));/*inserto hora -orden y guardo imag*/
    ...
    $bd->actualiza_(armaupdate('pestanhas_webinars',$campos," id_pesta='".$_POST["id_pesta"]."'",'POST'));/*actualizo*/
  }
  $bd->close();
  ...
  if($_GET["task"]=='edit'){
     $data_servicio=executesql("select * from pestanhas_webinars where id_pesta='".$_GET["id_pesta"]."'",0);
  }
?>
<!-- CK EDITOR -->
...
<?php $task_=$_GET["task"]; ?>
        <!-- form start -->
        <form id="registro" action="pestanhas_webinars.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
...
<?php 
  ...
  $ide = !isset($_GET['id_pesta']) ? implode(',', $_GET['id_pesta']) : $_GET['id_pesta'];
	 ...
	 $sql_="SELECT * FROM pestanhas_webinars WHERE id_pesta IN(".$ide.")";
	
	$pestanhas_webinars = executesql($sql_);
  if(!empty($pestanhas_webinars)){
    foreach($pestanhas_webinars as $row){
      // $pfile = 'files/images/pestanhas_webinars/'.$row['imagen']; if(file_exists($pfile) && !empty($row['imagen'])){ unlink($pfile); }
    }
  }

  $bd->actualiza_("DELETE FROM pestanhas_webinars WHERE id_pesta IN(".$ide.")");
  $bd->Commit();
  $bd->close();
  ...
  $pestanhas_webinars = executesql("SELECT * FROM pestanhas_webinars WHERE id_pesta IN (".$ide.")");
  if(!empty($pestanhas_webinars))
    foreach($pestanhas_webinars as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE pestanhas_webinars SET estado_idestado=".$state." WHERE id_pesta=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
  ...
  $sql = "SELECT cv.*,e.nombre AS estado FROM pestanhas_webinars cv INNER JOIN estado e ON cv.estado_idestado=e.idestado "; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="pestanhas_webinars.php";
?>
...
$(function(){
  checked();
  sorter();
  reordenar('pestanhas_webinars.php');
});
var mypage = "pestanhas_webinars.php";
</script>
<?php }else{ ?>
<div class="box-body">
      ...
      <div class="bg-gray-light">      
        <div class="col-sm-12">
          <?php   $sql_x=" select * from webinars where id_webinar='".$_GET["id_webinar"]."' "; 
						...
						echo "<h3 style='margin:0;' ><small> <b style='color:#333;'>Contenido de carta larga :</b> </small></h3>";
