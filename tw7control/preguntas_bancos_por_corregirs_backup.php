...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  ...
  $_POST["corregido"]= ($_GET["task"]=='insert')?1:  $_POST["corregido"]; // si se registra pasa como 1 
	...
	$campos=array('id_cate','titulo',array('titulo_rewrite',$urlrewrite),'puntos','corregido','descripcion','solucion','solucion_es_video','fecha_actualizacion','estado_idestado'); /*inserto campos principales*/
  if($_GET["task"]=='insert'){
		// echo "A";
...
<?php
  ...
  if(!empty($id_curso2)){
    foreach($id_curso2 as $row2){
      ...
      if(file_exists($pfile1) && !empty($row2['imagen'])){ unlink($pfile1); }
			
			$pfile2 = 'files/images/imagenes/'.$row2['imagen2'];
      if(file_exists($pfile2) && !empty($row2['imagen2'])){ unlink($pfile2); }
    }
  }
