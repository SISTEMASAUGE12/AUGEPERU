...
if($_GET["task"]=='finder'){
	if(!empty($_GET['fechabus_1']) && !empty($_GET['fechabus_2'])) {
			...
			where pp.estado_pago=1 and uu.idtipo_usu=4 and pp.tipo_venta=1   
		...
		//$sql.= " GROUP by pp.idusuario  ORDER BY n_ventas DESC";
		$sql.= "  ORDER BY nombre_corto asc ";
		...
		<?php 
			...
			while ($detalles = $paging->fetchResultado()): 
						...
						$sql_cliente_antiguo= 'select SUM(pe.tipo_venta=3) as total_ventas_sin_comision, sum(pe.total) as total_ventas_cliente_antiguos from pedidos pe where pe.estado_pago=1 and pe.estado_idestado=1  and pe.idusuario="'.$detalles["idusuario"].'" and pe.categoria_venta !=2  and pe.tipo_venta=3';
		...
		?>        
					...
					// include("lista_de_comision_actual.php");
						...
						<td class="cnone"> s/. <?php echo $comision_propia=$detalles["n_total_soles"]; ?></td>
