...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"respuestas_bancos","id_rpta","titulo_rewrite",$where);
	...
	$campos=array('id_pregunta','titulo',array('titulo_rewrite',$urlrewrite),'estado_rpta','fecha_actualizacion','estado_idestado'); /*inserto campos principales*/
 
	if($_GET["task"]=='insert'){
		// echo "HOLA 1 ";
    // $_POST['orden'] = _orden_noticia("","respuestas_bancos","");
    $_POST['orden'] = 2021;
		// echo " demoro en calcular nuevo orden, por el exceso de registros en Base de datos: 'preguntas_bancos' ";
      ...
      $campos = array_merge($campos,array('imagen'));
		...
		// echo var_dump(arma_insert('respuestas_bancos',array_merge($campos,array('orden','fecha_registro')),'POST'));
		// exit();
		
    $bd->inserta_(arma_insert('respuestas_bancos',array_merge($campos,array('orden','fecha_registro')),'POST'));/*inserto hora -orden y guardo imag*/
    ...
    $bd->actualiza_(armaupdate('respuestas_bancos',$campos," id_rpta='".$_POST["id_rpta"]."'",'POST'));/*actualizo*/
  }
	
  $bd->close();
  gotoUrl("index.php?page=".$_POST["nompage"]."&id_pregunta=".$_POST["id_pregunta"]."&module=".$_POST["nommodule"]."&parenttab=".$_POST["nomparenttab"]);
	
}elseif($_GET["task"]=='new' || $_GET["task"]=='edit'){
  if($_GET["task"]=='edit'){
     $data_servicio=executesql("select * from respuestas_bancos where id_rpta='".$_GET["id_rpta"]."'",0);
  }
?>
<!-- CK EDITOR -->
    ...
    <div class="col-md-12">
      ...
      <div class="box box-info">
        <div class="box-header with-border">
				 <?php   
							 $sql_x=" select p.titulo as pregunta  from preguntas_bancos p   WHERE    p.id_pregunta='".$_GET["id_pregunta"]."' "; 
							$datoscurso=executesql($sql_x,0); 
					?>				
          <h4 class="box-title"><?php echo ($_GET["task"]=="edit") ? 'Editar' : 'Agregando'; ?> respuestas_bancos: <b><?php echo $datoscurso["pregunta"]; ?> </b> </h4>
        </div><!-- /.box-header -->
<?php $task_=$_GET["task"]; ?>
        <!-- form start -->
        <form id="registro" action="respuestas_bancos.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
...
<?php
  ...
  $bd->actualiza_("DELETE FROM respuestas_bancos WHERE id_rpta IN(".$id_rpta.")");
  $bd->Commit();
  $bd->close();
  ...
  $respuestas_bancos = executesql("SELECT * FROM respuestas_bancos WHERE id_rpta IN (".$id_rpta.")");
  if(!empty($respuestas_bancos))
    foreach($respuestas_bancos as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE respuestas_bancos SET estado_idestado=".$state." WHERE id_rpta=".$id_rpta."");
  echo $state;
  $bd->Commit();
  $bd->close();
	
}elseif($_GET["task"]=='finder'){
  $sql = "SELECT cv.*,   p.titulo as pregunta, e.nombre AS estado FROM respuestas_bancos cv 
										INNER JOIN preguntas_bancos p ON p.id_pregunta=cv.id_pregunta  
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="respuestas_bancos.php";
?>
...
?>
    <tr  style="<?php echo $fondo_entregar; ?>" >
      ...
      <td>	<?php if( !empty($detalles['imagen']) ){  /* imagen respuestas_bancos */
...
$(function(){
  checked();
  sorter();
  reordenar('respuestas_bancos.php');
});
var mypage = "respuestas_bancos.php";
</script>
<?php }else{ ?>
<div class="box-body">
  <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
    <form action="javascript: fn_buscar();" id="frm_buscar" name="frm_buscar">
      ...
      <div class="bg-gray-light">      
        <div class="col-sm-12">
          <?php   $sql_x=" select p.id_pregunta, p.titulo as pregunta  from preguntas_bancos p  WHERE  p.id_pregunta='".$_GET["id_pregunta"]."' "; 
						$datoscurso=executesql($sql_x,0);
					...
					?>
						<h3 style='margin-top:0;padding-top:0;padding-bottom:10px;' ><small> 							
								<?php echo 'id: '.$datoscurso["id_pregunta"].' </br> <b style="color:#555;">Pregunta:</b> '.$datoscurso["pregunta"]; ?> </small></h3>
        </div>
        ...
        <div class="col-sm-2 ">          
         ...
         <div class="col-sm-2 criterio_mostrar"><div class="btn-eai">            
						<a href="index.php?page=preguntas_bancos&id_examen=<?php echo $_GET["id_examen"]; ?>&module=<?php echo $_GET["module"]; ?>&parenttab=<?php echo $_GET["parenttab"]; ?>" title="Regresar << " style="color:#fff;"><i class="fa fa-file" style="padding-right:5px;"></i>  Regresar</a> 
          </div></div>
