...
$hora = time();
$name_archivo  = 'Reporte_compras_clientes_'.$_GET['fecha_filtro_inicio'].'_al_'.$_GET['fecha_filtro_fin'];
$title  = 'Reporte de   Compras de Clientes: '.$_GET['fecha_filtro_inicio'].' al '.$_GET['fecha_filtro_fin'];
...
$cols   = array('FECHA','COD. COMPRA','ESTADO DE PAGO','TOTAL S/','COD. TRANSACCIÓN','FECHA VOUCHER','BANCO','DNI CLIENTE','CLIENTE','EMAIL','# ITEMS');
...
foreach($data as $detalles) {
	...
	if($detalles["estado_pago"]==2){ $detalles["estado_pago"]= "Por verificar";
	}elseif($detalles["estado_pago"]==1){ $detalles["estado_pago"]= "Aprobado";  
	}elseif($detalles["estado_pago"]==3){ $detalles["estado_pago"]= "Rechazado"; 
	}else{ $detalles["estado_pago"]= "#no fount."; 
	}
	
	if(!empty($detalles["codreferencia"])){
		$detalles['codigo_ope_off'] = $detalles["codreferencia"];
	}else{
		$detalles['codigo_ope_off'] = 'off-'.$detalles["codigo_ope_off"];
	}
