...
if($_POST["task"]=='atender_cliente_insertar_kardex' ){
		...
		if( !empty($validate) ){  /* si este vendedor es dueño de este cliente */
			...
			if( $validate['idusuario'] == $_SESSION["visualiza"]["idusuario"] ){ /* si es suyo */
				...
				echo "<script>
									...
									location.href='index.php?page=reportes_todos_clientes_ventas&module=Buscar%20si%20existe%20Cliente&parenttab=Reportes' );
							</script>";
			}
	...
	if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per'])){
			...
			$sql= "SELECT s.*, YEAR(s.fecha_registro) as anho, MONTH(s.fecha_registro) as mes, e.nombre AS estado, CONCAT(s.nombre,' ',s.ap_pa,' ',s.ap_ma )as suscritos, ep.titulo as especialidad , u.nomusuario as usuariox, SUM(estado_pago) as n_ventas    
			FROM suscritos s 
			INNER JOIN estado e ON s.estado_idestado=e.idestado 
				LEFT JOIN pedidos p ON s.id_suscrito=p.id_suscrito    
			...
			if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per'])){
				$stringlike=fn_filtro(substr($_GET['criterio_usu_per'], 0, 50));
					...
					$sql.= "  AND ( s.dni = '".$stringlike."' or s.telefono = '".$stringlike."'  or s.email = '".$stringlike."' )"; 
		...
		//  echo $sql;
			...
			$paging->ejecutar();
			$paging->pagina_proceso="reportes_todos_clientes_ventas.php";
...
?>
  ...
  <?php if($_SESSION["visualiza"]["idtipo_usu"] ==1 && $_SESSION["visualiza"]["idusuario"] ==6 ){  ?> 
		...
		<a href="javascript:fn_exportar('tipo_form=1&fecha_filtro_inicio=<?php echo $fecha_excel_inicio;?>&fecha_filtro_fin=<?php echo $_GET["fechabus_2"];?>','todos_clientes');" class="btn btn-primary"  > Excel [aplicando filtros]</a>
		
		<a href="../datausuario.php" class="btn btn-primary"  style="margin-left:20px;"> Excel [descargar todo]</a>
...
// $total_clientes=executesql("select count(*) as total_clientes from suscritos where estado_idestado=1 ");
// echo 'Total de registros: '.$total_clientes[0]['total_clientes'];
...
?>
	...
	<?php 	if( $_SESSION["visualiza"]["idtipo_usu"] ==1 ){   ?>
          <th class="sort">REGISTRADO POR</th>
	<?php } ?> 
          ...
          <th class="sort " style="background:green;color:#fff;">#COMPRAS</th>
...
?>        
	...
	<?php 	if( $_SESSION["visualiza"]["idtipo_usu"] ==1 ){   ?>		
        ...
        <td class=" text-center "><b><?php echo $detalles["n_ventas"]; ?></b> </td>
				<?php 
            ...
            <div class="row">
              <div class="col-sm-12">
                ...
                <div id="div_oculto" style="display: none;"></div>
						...
						<?php 
							$pagina_destino="reportes_todos_clientes_ventas";
...
<script>
var link = "reportes_todos_cliente";/*la s final se agrega en js fuctions*/
...
var ar = "al";
var id = "id_pedido";
var mypage = "reportes_todos_clientes_ventas.php";
</script>
<?php } ?>