...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"silabos_landing_bigs","id_silabo","titulo_rewrite",$where);
	...
	if($_GET["task"]=='insert'){
      ...
      $_POST['imagen'] = carga_imagen('files/images/silabos_landing_bigs/','imagen','','414','240');
      $campos = array_merge($campos,array('imagen'));
    }
		
    $_POST['orden'] = _orden_noticia("","silabos_landing_bigs","");
    $bd->inserta_(arma_insert('silabos_landing_bigs',array_merge($campos,array('orden')),'POST'));/*inserto hora -orden y guardo imag*/
		
		
  }else{
		
		if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $path = 'files/images/silabos_landing_bigs/'.$_POST['imagen_ant'];
      if( file_exists($path) && !empty($_POST['imagen_ant']) ) unlink($path);    
      $_POST['imagen'] = carga_imagen('files/images/silabos_landing_bigs/','imagen','','414','240');
      $campos = array_merge($campos,array('imagen'));
    }
    $bd->actualiza_(armaupdate('silabos_landing_bigs',$campos," id_silabo='".$_POST["id_silabo"]."'",'POST'));/*actualizo*/
  ...
  if($_GET["task"]=='edit'){
     $data_servicio=executesql("select * from silabos_landing_bigs where id_silabo='".$_GET["id_silabo"]."'",0);
  }
?>
<!-- CK EDITOR -->
...
<?php $task_=$_GET["task"]; ?>
        <!-- form start -->
        <form id="registro" action="silabos_landing_bigs.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
...
?>
          <div class="box-body">
            ...
            <div class="form-group">
              <label for="inputPassword3" class="col-sm-2 control-label">Modulo #</label>
              <div class="col-sm-10">
                <?php create_input("text","modulo",$data_servicio["modulo"]," form-control ",$table,' ');  ?>
                ...
                <?php /* 
						<div class="form-group">
							...
							<div class="col-sm-6">
								...
								?>
									<img src="<?php echo "files/images/silabos_landing_bigs/".$data_servicio["imagen"]; ?>" width="200" class="mgt15">
								<?php } ?> 
							</div>
						</div>		
								*/ ?>				
...
<?php
  ...
  $bd->actualiza_("DELETE FROM silabos_landing_bigs WHERE id_silabo IN (".$ide.")");
  $bd->Commit();
  $bd->close();
  ...
  $silabos_landing_bigs = executesql("SELECT * FROM silabos_landing_bigs WHERE id_silabo IN (".$ide.")");
  if(!empty($silabos_landing_bigs))
    foreach($silabos_landing_bigs as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE silabos_landing_bigs SET estado_idestado=".$state." WHERE id_silabo=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
	
}elseif($_GET["task"]=='finder'){
  $sql = "SELECT cv.*,e.nombre AS estado FROM silabos_landing_bigs cv INNER JOIN estado e ON cv.estado_idestado=e.idestado "; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="silabos_landing_bigs.php";
...
<?php while ($detalles = $paging->fetchResultado()): ?>
    <tr>
      ...
      <td><?php echo $detalles["modulo"]; ?></td>
...
$(function(){
  checked();
  sorter();
  reordenar('silabos_landing_bigs.php');
});
var mypage = "silabos_landing_bigs.php";
</script>
<?php }else{ ?>
<div class="box-body">
      ...
      <div class="bg-gray-light">      
        <div class="col-sm-12">
          <?php    $sql_x=" select * from landings_bigs where id_big='".$_GET["id_big"]."' "; 
						...
						echo "<h3 style='margin:0;' ><small> <b style='color:#333;'>Agenda del evento :</b> </small></h3>";
