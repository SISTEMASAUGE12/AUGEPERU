...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $title  = 'Reporte de Cursos del clientes: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','COD. COMPRA','COD. CURSO','CURSO','DNI','CLIENTE','EMAIL','CELULAR','ESTADO');
		$rows   = array('fecha_registro','codped','codcur','titcur','dni','suscritos','email','telefono','estado');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
			...
			if($row["estado"]==2){ $row["estado"]= "Pendiente";
