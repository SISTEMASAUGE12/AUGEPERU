...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $title  = 'Reporte de Comisiones del: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];
		...
		$cols   = array('USUARIO','BANCO','CUENTA','VENTAS PROPIAS CURSOS	','# COMPARTIDAS','S/ TOTAL COMISION','COMISIÓN CLIENTE ANTIGUO','VENTAS CERTIFICADOS',' S/ CERTIFICADOS ','VENTAS LIBROS',' S/ LIBROS' ,'TOTAL A PAGAR');
		$rows   = array('usuario','banco','cuenta_banco','n_ventas','n_compartidas','total_comision_neta','comision_cliente_antiguo','total_ventas_certificados','total_soles_certificados','total_ventas_libros','total_soles_libros','total_a_pagar');
  }
...
//			$detalles["comision"]= $comision_total_propia= 	$detalles["n_ventas"]*	$detalles["comision"]; // comision total propias
			...
			$sql_cliente_antiguo= 'select SUM(pe.tipo_venta=3) as total_ventas_sin_comision from pedidos pe where pe.estado_pago=1 and pe.estado_idestado=1 and pe.categoria_venta !=2 and pe.idusuario="'.$detalles["idusuario"].'" ';
			...
			$detalles["total_a_pagar"]=  		$detalles["total_comision_neta"]  + 	$detalles["comision_cliente_antiguo"] + $detalles["total_soles_certificados"] + $detalles["total_soles_libros"] ; // comison de venta spropias 
