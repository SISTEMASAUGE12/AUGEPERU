...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_ventas_pago_efectivo__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Ventas PAGO EFECTIVO: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','COD. COMPRA','ESTADO DE PEDIDO','TOTAL S/','COD. TRANSACCIÓN','COD. ORDEN','FECHA PAGO','DNI CLIENTE','CLIENTE','EMAIL','# ITEMS');
		$rows   = array('fecha_registro','id_pedido','estado_pago','total','codreferencia','codigo_ope_off','fecha_pago_off','dni','suscritos','email','articulos');
  }
