...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"pestanhas_landings_bigs_inicios","id_pesta","titulo_rewrite",$where);
  ...
  if($_GET["task"]=='insert'){
    ...
    $_POST['orden'] = _orden_noticia("","pestanhas_landings_bigs_inicios","");
    $bd->inserta_(arma_insert('pestanhas_landings_bigs_inicios',array_merge($campos,array('orden')),'POST'));/*inserto hora -orden y guardo imag*/
    ...
    $bd->actualiza_(armaupdate('pestanhas_landings_bigs_inicios',$campos," id_pesta='".$_POST["id_pesta"]."'",'POST'));/*actualizo*/
  }
  ...
  if($_GET["task"]=='edit'){
     $data_servicio=executesql("select * from pestanhas_landings_bigs_inicios where id_pesta='".$_GET["id_pesta"]."'",0);
  }
?>
<!-- CK EDITOR -->
...
<?php $task_=$_GET["task"]; ?>
        <!-- form start -->
        <form id="registro" action="pestanhas_landings_bigs_inicios.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
...
?>
          <div class="box-body">
            ...
            <div class="form-group">
							<label for="inputPassword3" class="col-sm-2 control-label">Link video VIMEO </br> * primero sale video luego imagen en caso existan las 2</label>
							<div class="col-sm-8">
...
<?php 
  ...
  $ide = !isset($_GET['id_pesta']) ? implode(',', $_GET['id_pesta']) : $_GET['id_pesta'];
	 ...
	 $sql_="SELECT * FROM pestanhas_landings_bigs_inicios WHERE id_pesta IN(".$ide.")";
	
	$pestanhas_landings_bigs_inicios = executesql($sql_);
  if(!empty($pestanhas_landings_bigs_inicios)){
    foreach($pestanhas_landings_bigs_inicios as $row){
      // $pfile = 'files/images/pestanhas_landings_bigs_inicios/'.$row['imagen']; if(file_exists($pfile) && !empty($row['imagen'])){ unlink($pfile); }
    }
  }

  $bd->actualiza_("DELETE FROM pestanhas_landings_bigs_inicios WHERE id_pesta IN(".$ide.")");
  $bd->Commit();
  $bd->close();
  ...
  $pestanhas_landings_bigs_inicios = executesql("SELECT * FROM pestanhas_landings_bigs_inicios WHERE id_pesta IN (".$ide.")");
  if(!empty($pestanhas_landings_bigs_inicios))
    foreach($pestanhas_landings_bigs_inicios as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE pestanhas_landings_bigs_inicios SET estado_idestado=".$state." WHERE id_pesta=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
  ...
  $sql = "SELECT cv.*,e.nombre AS estado FROM pestanhas_landings_bigs_inicios cv INNER JOIN estado e ON cv.estado_idestado=e.idestado "; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="pestanhas_landings_bigs_inicios.php";
?>
...
$(function(){
  checked();
  sorter();
  reordenar('pestanhas_landings_bigs_inicios.php');
});
var mypage = "pestanhas_landings_bigs_inicios.php";
</script>
<?php }else{ ?>
<div class="box-body">
      ...
      <div class="bg-gray-light">      
        <div class="col-sm-12">
          <?php   $sql_x=" select * from landings_bigs where id_big='".$_GET["id_big"]."' "; 
						...
						echo "<h3 style='margin:0;' ><small> <b style='color:#333;'>Contenido de Presentación, página de registro al evento :</b> </small></h3>";
...
<script>
var us = "seccion";
var link = "pestanhas_landings_bigs_inicio";
var ar = "la";
var l = "a";
var l2 = "a";
var pr = "La";
