...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"landing_big_x_expositores","id_pesta","titulo_rewrite",$where);
	
	$dir='files/images/landing_big_x_expositores/'.$_POST['id_big'].'/';

	$campos=array('id_big','titulo',array('titulo_rewrite',$urlrewrite),'descripcion','estado_idestado'); /*inserto campos principales*/
  if($_GET["task"]=='insert'){
		if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $_POST['imagen'] = carga_imagen($dir,'imagen','','180','180');
    ...
    $_POST['orden'] = _orden_noticia("","landing_big_x_expositores","");
    $bd->inserta_(arma_insert('landing_big_x_expositores',array_merge($campos,array('orden')),'POST'));/*inserto hora -orden y guardo imag*/
    ...
    $bd->actualiza_(armaupdate('landing_big_x_expositores',$campos," id_pesta='".$_POST["id_pesta"]."'",'POST'));/*actualizo*/
  }
  $bd->close();
  ...
  if($_GET["task"]=='edit'){
     $data_servicio=executesql("select * from landing_big_x_expositores where id_pesta='".$_GET["id_pesta"]."'",0);
  }
?>
<!-- CK EDITOR -->
      ...
      <div class="box box-info">
        <div class="box-header with-border">
				 <?php   $sql_x=" select * from landings_bigs  where id_big='".$_GET["id_big"]."' "; 
					...
					?>				
          <h4 class="box-title"><?php echo ($_GET["task"]=="edit") ? 'Editar' : 'Agregando'; ?> Expositores del  evento: <b><?php echo $datoscurso["titulo"]; ?> </b> </h4>
        </div><!-- /.box-header -->
<?php $task_=$_GET["task"]; ?>
        <!-- form start -->
        <form id="registro" action="landing_big_x_expositores.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
                    ...
                    <?php create_input("textarea","descripcion",$data_servicio["descripcion"],"",$table,$agregado);  ?>
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								...
								?>
									<img src="<?php echo "files/images/landing_big_x_expositores/".$data_servicio['id_big']."/".$data_servicio["imagen"]; ?>" width="200" class="mgt15">
								<?php } ?> 
							</div>
						</div>

						<div class="form-group">
							<label for="inputPassword3" class="col-sm-2 control-label">Imágen logotipo : 140px ancho * 40px alto</label>
							<div class="col-sm-6">
								<input type="file" name="imagen_2" id="imagen_2" class="form-control">
								<?php create_input("hidden","imagen_ant",$data_servicio["imagen_2"],"",$table,$agregado); 
									if($data_servicio["imagen_2"]!=""){ 
								?>
									<img src="<?php echo "files/images/landing_big_x_expositores/".$data_servicio['id_big']."/".$data_servicio["imagen_2"]; ?>" width="200" class="mgt15">
								<?php } ?> 
							</div>
						</div>
...
<?php 
  ...
  $ide = !isset($_GET['id_pesta']) ? implode(',', $_GET['id_pesta']) : $_GET['id_pesta'];
	 ...
	 $sql_="SELECT * FROM landing_big_x_expositores WHERE id_pesta IN(".$ide.")";
	
	$landing_big_x_expositores = executesql($sql_);
  if(!empty($landing_big_x_expositores)){
    foreach($landing_big_x_expositores as $row){
      // $pfile = 'files/images/landing_big_x_expositores/'.$row['imagen']; if(file_exists($pfile) && !empty($row['imagen'])){ unlink($pfile); }
    }
  }

  $bd->actualiza_("DELETE FROM landing_big_x_expositores WHERE id_pesta IN(".$ide.")");
  $bd->Commit();
  $bd->close();
  ...
  $landing_big_x_expositores = executesql("SELECT * FROM landing_big_x_expositores WHERE id_pesta IN (".$ide.")");
  if(!empty($landing_big_x_expositores))
    foreach($landing_big_x_expositores as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE landing_big_x_expositores SET estado_idestado=".$state." WHERE id_pesta=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
  ...
  $sql = "SELECT cv.*,e.nombre AS estado FROM landing_big_x_expositores cv INNER JOIN estado e ON cv.estado_idestado=e.idestado "; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="landing_big_x_expositores.php";
?>
...
$(function(){
  checked();
  sorter();
  reordenar('landing_big_x_expositores.php');
});
var mypage = "landing_big_x_expositores.php";
</script>
<?php }else{ ?>
<div class="box-body">
    ...
    <form action="javascript: fn_buscar();" id="frm_buscar" name="frm_buscar">
      ...
      <div class="bg-gray-light">      
        <div class="col-sm-12">
          <?php   $sql_x=" select * from landings_bigs  where id_big='".$_GET["id_big"]."' "; 
						$datoscurso=executesql($sql_x,0);
						// $volver_al_curso="index.php?page=landing_big_x_expositores&module=".$_GET["module"]."&parenttab=".$_GET["parenttab"]."";
...
<script>
var us = "ponente";
var link = "landing_big_x_expositore";
var ar = "la";
var l = "a";
var l2 = "a";
var pr = "La";
