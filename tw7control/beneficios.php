...
if($_GET["task"]=='neworden'){
  ...
  $criterio_Orden ="";
  nuevoorden($orden_actual, $orden_nuevo, $tipo, $id_del_registro_actual, "beneficios", "id", $criterio_Orden);    
  $bd->close();

}elseif($_GET["task"]=='insert' || $_GET["task"]=='update'){
  $bd=new BD;
  $norden=_orden_noticia("","beneficios","");
  ...
  if($_GET["task"]=='insert'){
      ...
      $_POST['imagen'] = carga_imagen('files/images/beneficios/','imagen','','51','51');
      $campos = array_merge($campos,array('imagen'));
      }
    $bd->inserta_(arma_insert('beneficios',$campos,'POST'));
  }else{
      if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $path = 'files/images/beneficios/'.$_POST['imagen_ant'];
      if( file_exists($path) && !empty($_POST['imagen_ant']) ) unlink($path);    
      $_POST['imagen'] = carga_imagen('files/images/beneficios/','imagen','','51','51');
      $campos = array_merge($campos,array('imagen'));
    }
     $bd->actualiza_(armaupdate('beneficios',$campos," id='".$_POST["id"]."'",'POST'));/*actualizo*/
  }
  ...
  if($_GET["task"]=='edit'){
     $data_producto=executesql("select * from beneficios where id='".$_GET["id"]."'",0);
  }
?>
    ...
    <div class="col-md-12">          
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">
                <?php echo ($_GET["task"]=="edit") ? 'Editar' : 'Nueva'; ?> beneficios</h3>
                <p style="color:red;">* Esta configuración afecta a la portada de la web. Ruta: <?php echo $_dominio; ?></p>

				<!-- <p>(*) Opcional, no indispensable</p>-->
            </div>
<?php $task_=$_GET["task"]; ?>            
            <form id="registro" action="beneficios.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST"  enctype="multipart/form-data">
<?php 
...
?>
              <div class="box-body">
                ...
                <div class="form-group">
                  <label for="inputPassword3" class="col-sm-2 control-label">IMAGEN ICONO</label>
                  <div class="col-sm-6">
                    <input type="file" name="imagen" id="imagen" class="form-control">
                    <?php create_input("hidden","imagen_ant",$data_producto["imagen"],"",$table,$agregado); 
                      if($data_producto["imagen"]!=""){ 
                    ?>
                      <img src="<?php echo "files/images/beneficios/".$data_producto["imagen"]; ?>" width="200" class="mgt15">
                    <?php } ?> 
                      <small style="color:red">Recomendado: 51px x 51px</small>
                  </div>
                </div>
...
<?php
  ...
  $categoria = executesql("SELECT imagen FROM beneficios WHERE id IN(".$ide.")");
  if(!empty($categoria)){
    foreach($categoria as $row){
      $pfile = 'files/images/beneficios/'.$row['imagen']; 
      if(file_exists($pfile) && !empty($row['imagen'])) unlink($pfile);
    }
  }
  $bd->actualiza_("DELETE FROM beneficios WHERE id IN(".$ide.")"); 
  $bd->Commit();
  $bd->close();
  ...
  foreach ($_GET['order'] as $order => $item) {
    $orden = $orden + 1;
    $num_afect=$bd->actualiza_("UPDATE beneficios SET orden= ".$orden." WHERE id = ".$item."");
  }
  ...
  $categoria = executesql("SELECT * FROM beneficios WHERE id IN (".$ide.")");
  if(!empty($categoria))
  ...
  $num_afect=$bd->actualiza_("UPDATE beneficios SET estado_idestado=".$state." WHERE id=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
  ...
  $sql.= "SELECT c.*,e.nombre AS estado FROM beneficios c INNER JOIN estado e ON c.estado_idestado=e.idestado";
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="beneficios.php";
?>
...
<?php while ($detalles = $paging->fetchResultado()): ?>
                <tr id="order_<?php echo $detalles["id"]; ?>">
                  <td><input type="checkbox" name="chkDel[]" class="chkDel" value="<?php echo $detalles["id"]; ?>"></td>
                    ...
                    <img src="<?php echo "files/images/beneficios/".$detalles["imagen"]; ?>" alt="<?php echo $detalles["nombre"]; ?>" class="img-responsive">
