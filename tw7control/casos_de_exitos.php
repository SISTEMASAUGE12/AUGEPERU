...
if($_GET["task"]=='neworden'){
  ...
  $criterio_Orden =" ";
  nuevoorden($orden_actual, $orden_nuevo, $tipo, $id_del_registro_actual, "casos_de_exitos", "ide", $criterio_Orden);    
  $bd->close();
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"casos_de_exitos","ide","titulo_rewrite",$where);
  // $campos=array('titulo', array('titulo_rewrite',$urlrewrite),'tipo','cargo','descripcion',"link","estado_idestado"); 
  $campos=array('titulo', array('titulo_rewrite',$urlrewrite),"link","estado_idestado"); 
  ...
  if($_GET["task"]=='insert'){
    if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $_POST['imagen'] = carga_imagen('files/images/casos_de_exitos/','imagen','','','');
      $campos = array_merge($campos,array('imagen'));
    }
    if(isset($_FILES['imagen2']) && !empty($_FILES['imagen2']['name'])){
      $_POST['imagen2'] = carga_imagen('files/images/casos_de_exitos/','imagen2','','','');
    ...
    // }
    $_POST['orden'] = _orden_noticia("","casos_de_exitos","");
    $bd->inserta_(arma_insert('casos_de_exitos',array_merge($campos,array('orden','idusuario')),'POST'));
		
  }else{
    if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $path = 'files/images/casos_de_exitos/'.$_POST['imagen_ant'];
      if( file_exists($path) && !empty($_POST['imagen_ant']) ) unlink($path);    
      $_POST['imagen'] = carga_imagen('files/images/casos_de_exitos/','imagen','','','');
      $campos = array_merge($campos,array('imagen'));
    }
    if(isset($_FILES['imagen2']) && !empty($_FILES['imagen2']['name'])){
      $path = 'files/images/casos_de_exitos/'.$_POST['imagen_ant2'];
    ...
    // }
		...
		// echo var_dump(armaupdate('casos_de_exitos',$campos," ide='".$_POST["ide"]."'",'POST'));
		// exit();
		
    $bd->actualiza_(armaupdate('casos_de_exitos',$campos," ide='".$_POST["ide"]."'",'POST'));/*actualizo*/
  ...
  if($_GET["task"]=='edit'){
     $data_producto=executesql("select * from casos_de_exitos where ide='".$_GET["ide"]."'",0);
  }
?>
    ...
    <div class="col-md-12">
          ...
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title"> Casos de éxito </h3>
            </div><!-- /.box-header -->
            <p style="color:red;">*Ubicacion:  Esta configuración afecta a la portada de la web. Ruta: <?php echo $_dominio; ?> => dentro de cada detalle de un curso </p>

<?php $task_=$_GET["task"]; ?>
            <!-- form start -->
            <form id="registro" action="casos_de_exitos.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-6">
                    <?php create_input("text","titulo",$data_producto["titulo"],"form-control",$table,"",$agregado); ?>
								...
								<div class="form-group">
                  <label for="inputPassword3" class="col-sm-2 control-label">Imagen: (380px ancho *  220px alto) </label>
                  <div class="col-sm-6">
                    <input type="file" name="imagen" id="imagen" class="form-control">
                    <?php create_input("hidden","imagen_ant",$data_producto["imagen"],"",$table,$agregado); 
                      if($data_producto["imagen"]!=""){ 
                    ?>
                      <img src="<?php echo "files/images/casos_de_exitos/".$data_producto["imagen"]; ?>" width="200" class="mgt15">
                    <?php } ?> 
                  </div>
                </div>
...
<?php
  ...
  $casos_de_exitos = executesql("SELECT * FROM casos_de_exitos WHERE ide IN(".$ide.")");
  if(!empty($casos_de_exitos)){
    foreach($casos_de_exitos as $row){
      $pfile = 'files/images/casos_de_exitos/'.$row['imagen']; if(file_exists($pfile) && !empty($row['imagen'])){ unlink($pfile); }
      // $pfile = 'files/images/casos_de_exitos/'.$row['imagen2']; if(file_exists($pfile) && !empty($row['imagen2'])){ unlink($pfile); }
     }
    }  
  $bd->actualiza_("DELETE FROM casos_de_exitos WHERE ide IN(".$ide.")");
  $bd->Commit();
  $bd->close();
  ...
  foreach ($_GET['order'] as $order => $item) {
    $orden = $orden + 1;
    $bd->actualiza_("UPDATE casos_de_exitos SET orden= ".$orden." WHERE ide = ".$item."");
  }
  $bd->Commit();
  $bd->close();
  ...
  if(!empty($casos_de_exitos))
    foreach($casos_de_exitos as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE casos_de_exitos SET estado_idestado=".$state." WHERE ide=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
  ...
  $sql = "SELECT p.*, e.nombre as estado, u.nomusuario as usuario 
						FROM casos_de_exitos p 
  ...
  if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per'])){
    ...
    $sql.= " where p.titulo LIKE '".$stringlike."%' "; /* filtra si la empieza empieza con esto.. */
  }
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="casos_de_exitos.php";
?>
            <table id="example1" class="table table-bordered table-striped">
              <thead>
                <tr role="row">
                  ...
                  <th class="unafbe ">IMG</th>
                  <th class="unafbe cnone">LINK</th>
...
<?php while ($detalles = $paging->fetchResultado()): ?>
                <tr id="order_<?php echo $detalles["ide"]; ?>">
                  ...
                  <td class=" cnone "><?php echo $detalles["titulo"]; ?></td> 
                    ...
                    <img src="<?php echo "files/images/casos_de_exitos/".$detalles["imagen"]; ?>" alt="<?php echo $detalles["nombre"]; ?>" class="img-responsive">
                    <?php }else{ echo "Not Image."; } ?>
                  </td>									
                  <td class=" cnone "><?php echo $detalles["link"]; ?></td>    									
                  <td class=" "><a href="javascript: fn_estado('<?php echo $detalles["ide"]; ?>')"><?php echo $detalles["estado"]; ?></a></td>
...
$(function(){
  checked();
  sorter();
  reordenar('casos_de_exitos.php');
});
var mypage = "casos_de_exitos.php";
</script>
