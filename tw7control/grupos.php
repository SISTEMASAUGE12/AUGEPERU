...
if($_GET["task"]=='neworden'){
  ...
  $criterio_Orden =" ";
  nuevoorden($orden_actual, $orden_nuevo, $tipo, $id_del_registro_actual, "grupos", "ide", $criterio_Orden);    
  $bd->close();
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"grupos","ide","titulo_rewrite",$where);
  // $campos=array('titulo', array('titulo_rewrite',$urlrewrite),'descripcion',"enlace","estado_idestado"); 
  $campos=array('titulo', array('titulo_rewrite',$urlrewrite),"enlace","estado_idestado"); 
  
  if($_GET["task"]=='insert'){
    if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $_POST['imagen'] = carga_imagen('files/images/grupos/','imagen','','','');
      $campos = array_merge($campos,array('imagen'));
    }
    if(isset($_FILES['imagen2']) && !empty($_FILES['imagen2']['name'])){
      $_POST['imagen2'] = carga_imagen('files/images/grupos/','imagen2','','','');
    ...
    // }
    $_POST['orden'] = _orden_noticia("","grupos","");
    $bd->inserta_(arma_insert('grupos',array_merge($campos,array('orden')),'POST'));
  }else{
    if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $path = 'files/images/grupos/'.$_POST['imagen_ant'];
      if( file_exists($path) && !empty($_POST['imagen_ant']) ) unlink($path);    
      $_POST['imagen'] = carga_imagen('files/images/grupos/','imagen','','','');
      $campos = array_merge($campos,array('imagen'));
    }
    if(isset($_FILES['imagen2']) && !empty($_FILES['imagen2']['name'])){
      $path = 'files/images/grupos/'.$_POST['imagen_ant2'];
    ...
    // if(isset($_FILES['archivo']) && !empty($_FILES['archivo']['name'])){
      ...
      // $campos = array_merge($campos,array('archivo'));
    // }
    $bd->actualiza_(armaupdate('grupos',$campos," ide='".$_POST["ide"]."'",'POST'));/*actualizo*/
  }
  $bd->close();
  ...
  if($_GET["task"]=='edit'){
     $data_producto=executesql("select * from grupos where ide='".$_GET["ide"]."'",0);
  }
?>
...
<?php $task_=$_GET["task"]; ?>
            <!-- form start -->
            <form id="registro" action="grupos.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data">
<?php 
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-6">
                    ...
                    ?>
                      <img src="<?php echo "files/images/grupos/".$data_producto["imagen"]; ?>" width="200" class="mgt15">
                    <?php } ?> 
                  </div>
                </div>
...
<?php
  ...
  $grupos = executesql("SELECT * FROM grupos WHERE ide IN(".$ide.")");
  if(!empty($grupos)){
    foreach($grupos as $row){
      $pfile = 'files/images/grupos/'.$row['imagen']; if(file_exists($pfile) && !empty($row['imagen'])){ unlink($pfile); }
      // $pfile = 'files/images/grupos/'.$row['imagen2']; if(file_exists($pfile) && !empty($row['imagen2'])){ unlink($pfile); }
     }
    }  
  $bd->actualiza_("DELETE FROM grupos WHERE ide IN(".$ide.")");
  $bd->Commit();
  $bd->close();
  ...
  foreach ($_GET['order'] as $order => $item) {
    $orden = $orden + 1;
    $bd->actualiza_("UPDATE grupos SET orden= ".$orden." WHERE ide = ".$item."");
  }
  $bd->Commit();
  $bd->close();
  ...
  if(!empty($grupos))
    foreach($grupos as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE grupos SET estado_idestado=".$state." WHERE ide=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
  ...
  $sql = "SELECT p.*, e.nombre as estado FROM grupos p Inner join estado e on p.estado_idestado=e.idestado "; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="grupos.php";
?>
...
<?php while ($detalles = $paging->fetchResultado()): ?>
                <tr id="order_<?php echo $detalles["ide"]; ?>">
                  ...
                  <td class="cnone">
                    ...
                    <img src="<?php echo "files/images/grupos/".$detalles["imagen"]; ?>"  class="img-responsive">
                    <?php }else{ echo "Not Image."; } ?>
                  </td>    

                  <td class="cnone "> <?php echo $detalles["enlace"]; ?></td>                
...
$(function(){
  checked();
  sorter();
  reordenar('grupos.php');
});
var mypage = "grupos.php";
</script>
