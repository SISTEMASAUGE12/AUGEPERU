...
if($_GET["task"]=='finder'){
  ...
  // WHERE sc.id_tipo=1 and sc.estado=1 "; 
	...
	$sql= "SELECT sc.*, YEAR(sc.fecha_registro) as anho, MONTH(sc.fecha_registro) as mes ,s.email as email,  CONCAT(s.nombre,' ',s.ap_pa,' ',s.ap_ma )as suscritos , s.dni as dni, c.titulo as curso, c.codigo as codigo, s.telefono , esp.titulo as especialidad, p.total , u.nombre_corto , pes.tipo_pago 
	FROM suscritos_x_cursos sc 
  INNER JOIN estado e ON sc.estado_idestado=e.idestado 
  INNER JOIN suscritos s ON sc.id_suscrito=s.id_suscrito 
  LEFT JOIN pedidos pes ON sc.id_pedido=pes.id_pedido  
...
?>
  <table id="example1" class="table table-bordered table-striped">
    ...
    <!-- 
    <a href="javascript:fn_exportar('tipo_form=1&fecha_filtro_inicio=<?php echo $fecha_excel_inicio;?>&fecha_filtro_fin=<?php echo $_GET["fechabus_2"];?>','clientes_cursos');" class="btn btn-primary excel "  > Excel  [aplicando filtros x mes]</a>
    -->

    <a href="javascript:fn_exportar('fecha_filtro_inicio=<?php echo $fecha_excel_inicio;?>&fecha_filtro_fin=<?php echo $_GET["fechabus_2"];?>','clientes_cursos');" class="btn btn-primary excel "  > Excel  [aplicando filtros x mes]</a>
		
		<a href="javascript:fn_exportar_todo('excel_todo_clientes_x_cursos.php');"  class="btn btn-primary"  style="margin-left:20px;"> Excel [descargar todo]</a>
...
if(!empty($stringlike)  || !empty($sql_fecha_total) ){ 
 $sql_t="select count(*) as total  
								from suscritos_x_cursos sc 
								INNER JOIN cursos c ON sc.id_curso= c.id_curso 
								where sc.estado_idestado=1 and sc.estado=1  ".$sql_fecha_total."  and  ( c.codigo LIKE '".$stringlike."%' or c.titulo LIKE '".$stringlike."%' ) ";
