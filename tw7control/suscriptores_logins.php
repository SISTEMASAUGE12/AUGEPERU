...
if($_GET["task"]=='buscardni'){
	...
	$campos=array('nombre','ap_pa','ap_ma','dni','telefono',"estado_idestado",'id_escala_mag'); 	
...
<?php $task_=$_GET["task"]; ?>            
            <form action="suscriptores_logins.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" autocomplete="OFF" onsubmit="return aceptar()">
<?php 
...
?>
		...
		<fieldset id="field_client" >
				...
				<div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">ESCALA</label>
                  <div class="col-sm-6">
                    <?php  crearselect("id_escala_mag","select id_escala_mag, titulo  from escala_magisteriales where estado_idestado=1 order by titulo asc ",'class="poppi " required ',$usuario["id_escala_mag"],"  -- escala -- ");  ?>
                  </div>
                </div>		
...
var mypage = "suscriptores_logins.php";
</script>
...
<?php
  ...
  if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per']) ){
	...
	*/
	...
	if( !empty($_GET['id_tipo_cliente']) && isset($_GET['id_tipo_cliente']) ) {
		$sql.= " and c.id_tipo_cliente= '".$_GET['id_tipo_cliente']."' ";  
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="suscriptores_logins.php";
?>
...
<?php }//if meses ?>
				<tr>
					...
					<td class=" cnone "><?php echo $detalles["usuariox"]; ?></td>
							<?php if( !empty( $detalles["fecha_atencion_algoritmo"] )  ){  ?>
...
<script>
$(function(){
  // reordenar('suscriptores_logins.php');
  checked();
  // sorter();
});
</script>
...
<script>
var link = "suscriptores_login";/*la s final se agrega en js fuctions*/
