...
$hora = time();
$name_archivo  = 'Reporte_solicitudes_coautorias__'.$_GET['fecha_filtro_inicio'].'_al_'.$_GET['fecha_filtro_fin'];
$title  = 'Reporte de Solicitudes CoAutorías:: '.$_GET['fecha_filtro_inicio'].' al '.$_GET['fecha_filtro_fin'];
...
foreach($data as $detalles) {
	...
	if($detalles["estado"]==1){ $detalles["estado"] = 'Entregado';
	}elseif($detalles["estado"]==2){ $detalles["estado"] = 'Pendiente';
	}elseif($detalles["estado"]==3){ $detalles["estado"] = 'Rechazado';
	}elseif($detalles["estado"]==4){ $detalles["estado"] = 'Procesando';
	}elseif($detalles["estado"]==5){ $detalles["estado"] = 'Enviado';
	}
	echo "<tr>";
		...
		echo "<td>".$detalles['depa']."</td>";  				
		echo "<td>".$detalles['prov']."</td>";  				
