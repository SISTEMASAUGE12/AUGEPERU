...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  $bd=new BD;  
	$campos=array('id_suscrito','id_certificado','id_curso');  
    ...
    $_POST['orden'] = _orden_noticia("","suscritos_x_certificados","");
		...
		$campos=array_merge($campos,array('id_pedido','orden','id_tipo','precio','fecha_registro','estado','estado_entrega','estado_idestado'));
  ...
  if($_GET["task"]=='edit'){
    $data_producto=executesql("select d.*  from suscritos_x_certificados d INNER JOIN suscritos s  ON s.id_suscrito=d.id_suscrito  INNER JOIN estado e ON d.estado_idestado=e.idestado where ide='".$_GET["ide"]."' ",0);
  }
?>
...
<?php $task_=$_GET["task"]; ?>
      <form action="suscriptores_certificados_solicitados.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" autocomplete="OFF" onsubmit="return aceptar()">
<?php
					...
					<div class="form-group">
					  <label for="inputEmail3" class="col-sm-2 control-label">CURSO</label>
						<div class="col-sm-3 criterio_buscar">
									<?php crearselect("id_curso", "select id_curso, CONCAT(codigo,' - ',titulo) as titulo_x from cursos  order by codigo asc ", 'class="form-control" requerid  onchange="javascript:display(\'suscriptores_certificados_solicitados.php\',this.value,\'cargar_certificados_del_curso\',\'id_certificado\')"', $data_producto["id_curso"], "-- curso --"); ?>
						</div>
					</div>
...
<?php 
  ...
  $sql= "SELECT pp.*,YEAR(pp.fecha_registro) as anho, MONTH(pp.fecha_registro) as mes, s.email as email,  s.dni , CONCAT(cc.id_certificado,' - ',cc.titulo) as certificado,  CONCAT(c.codigo,' - ',c.titulo) as curso   
	FROM solicitudes pp 
  INNER JOIN cursos c ON pp.id_curso =c.id_curso  
  INNER JOIN certificados cc ON pp.id_certificado =cc.id_certificado 
 ...
 // echo  $sql; 
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="suscriptores_certificados_solicitados.php";
?>
...
?>
          ...
          <th class="sort cnone">AGENCIA</th>
          <th class="sort cnone">FCH. ENVIO</th>
          <th class="sort cnone">FCH. ENTREGA</th>
...
?>        
       <tr style="<?php echo $fondo_entregar; ?>">
        ...
        <td class="cnone"><?php echo $detalles['agencia']; ?></td>        
        <td class="cnone"><?php echo $detalles['fecha_envio']; ?></td>        
        <td class="cnone"><?php echo $detalles['fecha_entrega']; ?></td>        
				<td class="cnone">
					<a  style="color:#333;font-weight:800;">
						<?php if($detalles["estado"]==2){ echo "Pendiente";
						}elseif($detalles["estado"]==3){ echo "Rechazado";  
						}elseif($detalles["estado"]==1){ echo "Entregado";  
...
<script>
$(function(){
  // reordenar('suscriptores_certificados_solicitados.php');
  // checked();
  // sorter();
});
</script>
					...
					-->
								...
								<div class="col-sm-3 criterio_buscar">
								 ...
								 <div class="col-sm-3 criterio_mostrar">
										<?php create_input('date', 'fechabus', '', "form-control pull-right", $table, ''); ?>
								</div>
...
<script>
var link = "suscriptores_certificados_solicitado";/*la s final se agrega en js fuctions*/
...
var ar = "al";
var id = "ide";
var mypage = "suscriptores_certificados_solicitados.php";
</script>
<?php } ?>