...
if($_GET["task"]=='finder'){
  ...
  INNER JOIN especialidades ep ON s.id_especialidad=ep.id_especialidad ";
  
  if (isset($_GET['criterio_mostrar'])) $porPagina=$_GET['criterio_mostrar'];
  if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per'])){
    $stringlike=fn_filtro(substr($_GET['criterio_usu_per'], 0, 16));
      $pos = strpos($sql, 'WHERE');
      $sql.= (($pos === false) ? "WHERE" : "AND" )." ( s.dni LIKE '".$stringlike."%' or s.nombre LIKE '".$stringlike."%' or s.email LIKE '".$stringlike."%' )"; 
  }else{
...
<?php  if($_SESSION["visualiza"]["idtipo_usu"] ==1){ ?>	
		...
		<a href="javascript:fn_exportar('tipo_form=1&fecha_filtro_inicio=<?php echo $fecha_excel_inicio;?>&fecha_filtro_fin=<?php echo $_GET["fechabus_2"];?>','todos_clientes');" class="btn btn-primary"  > Excel</a>
<?php } ?>
