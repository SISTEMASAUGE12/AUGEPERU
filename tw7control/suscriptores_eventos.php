...
<?php $task_=$_GET["task"]; ?>            
            <form action="suscriptores_eventos.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" autocomplete="OFF" onsubmit="return aceptar()">
<?php 
...
<?php
  ...
  // sino esta q usa el buscador 
	$sql61 = executesql("SELECT s.id_suscrito, count(s.id_suscrito) c FROM pedidos p INNER JOIN suscritos s ON p.id_suscrito=s.id_suscrito  WHERE s.idusuario='".$_SESSION["visualiza"]["idusuario"]."' GROUP BY s.id_suscrito HAVING c >= 1");
	$conte = '';
	if(!empty($sql61)){ 
		...
		$sql62 = " and  s.id_suscrito NOT IN (".$ulti.")   ";

	}else{   // sino existe nada concateno esto 
	
		$sql62 = "  ";
	}

	$sql= "SELECT s.*, YEAR(s.fecha_registro) as anho, MONTH(s.fecha_registro) as mes, e.nombre AS estado, CONCAT(s.nombre,' ',s.ap_pa,' ',s.ap_ma ) as suscritos, ep.titulo as especialidad , esc.titulo as escala , u.nomusuario as usuario 
	FROM suscritos s 
	LEFT JOIN usuario u ON s.idusuario = u.idusuario  
	INNER JOIN estado e ON s.estado_idestado=e.idestado 
	LEFT JOIN escala_magisteriales esc ON s.id_escala_mag=esc.id_escala_mag   
	LEFT JOIN especialidades ep ON s.id_especialidad=ep.id_especialidad 
		WHERE  ( ( s.dni is NULL or s.dni='') or (s.id_especialidad is NULL or s.id_especialidad ='' ))  and  (s.idusuario='".$_SESSION["visualiza"]["idusuario"]."' or s.idusuario is NULL  ) ".$sql62;
  ...
  if(isset($_GET['criterio_usu_per']) && !empty($_GET['criterio_usu_per']) ){
    ...
    $sql.= " AND (ap_pa = '".$stringlike."' OR s.ap_ma = '".$stringlike."' OR s.telefono = '".$stringlike."' OR s.dni = '".$stringlike."' OR s.email = '".$stringlike."' OR s.nombre = '".$stringlike."' OR s.id_suscrito = '".$stringlike."' )";

  }else{
			if( empty($_GET['fechabus_1']) && empty($_GET['fechabus_2']) ) {
				$sql .= " AND DATE(s.fecha_registro) = '" . fecha_hora(1) . "'";
			}
	}
	...
	*/
	...
	if(!empty($_GET['id_canal']) ) {
			$sql .= " AND s.id_canal='".$_GET['id_canal']."'  ";		
	}
	if(!empty($_GET['id_pais']) ) {
			$sql .= " AND s.id_pais='".$_GET['id_pais']."'  ";		
	}
	
	if(!empty($_GET['fechabus_1']) && !empty($_GET['fechabus_2'])) {
			$sql .= " AND DATE(s.fecha_registro)  BETWEEN  DATE('".$_GET['fechabus_1']."')  and DATE('".$_GET['fechabus_2']."')  ";		
	}
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="suscriptores_eventos.php";
?>
...
<script>
$(function(){
  // reordenar('suscriptores_eventos.php');
  checked();
  // sorter();
});
</script>
              ...
              <div class="bg-gray-light">
                <div class="col-sm-12" style="padding-bottom:20px;">									
									...
									<div class="col-sm-2  btn-eai ">                  
										<a href="https://www.educaauge.com/tw7control/index.php?page=suscriptores_eventos&module=Contactar_Ahora&parenttab=Mis%20Clientes" CLASS="btn " style="color:#fff;">Refrescar</a>
									</div>
...
<script>
var link = "suscriptores_toky";/*la s final se agrega en js fuctions*/
...
var ar = "al";
var id = "id_suscrito";
var mypage = "suscriptores_eventos.php";
</script>
<?php } ?>