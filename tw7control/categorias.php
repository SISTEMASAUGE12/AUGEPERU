...
if($_GET["task"]=='neworden'){
  ...
  $criterio_Orden =" ";
  nuevoorden($orden_actual, $orden_nuevo, $tipo, $id_del_registro_actual, "categorias", "id_cat", $criterio_Orden);    
  $bd->close();
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"categorias","id_cat","titulo_rewrite",$where);
	
  $_POST['idusuario']=$_SESSION["visualiza"]["idusuario"];
  
  $campos=array('titulo', array('titulo_rewrite',$urlrewrite),'detalle',"estado_idestado"); 
  
  echo "H_3";
  if($_GET["task"]=='insert'){
    if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $_POST['imagen'] = carga_imagen('files/images/categorias/','imagen','','282','165');
    ...
    // }
    $_POST['orden'] = _orden_noticia("","categorias","");
    $_POST['fecha_registro'] = fecha_hora(2);
    $bd->inserta_(arma_insert('categorias',array_merge($campos,array('orden','id_tipo','fecha_registro','idusuario')),'POST'));
    ...
    $bd->actualiza_(armaupdate('categorias',$campos," id_cat='".$_POST["id_cat"]."'",'POST'));/*actualizo*/
  }
  $bd->close();
  ...
  if($_GET["task"]=='edit'){
     $data_producto=executesql("select * from categorias where id_cat='".$_GET["id_cat"]."'",0);
  }
?>
  ...
  <div class="row">
    <div class="col-md-12">          
          <div class="box box-info">
            <div class="box-header with-border"><h3 class="box-title">Categorias </h3>
...
<?php $task_=$_GET["task"]; ?>            
            <form id="registro" action="categorias.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data"  onsubmit="return aceptar()">
<?php 
...
?>
              <div class="box-body">								
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-6">
                    <?php create_input("text","detalle",$data_producto["detalle"],"form-control",$table,"",$agregado); ?>
                  </div>
                </div>	
							 <div class="form-group">
                  ...
                  <div class="col-sm-6">
                    ...
                    ?>
                      <img src="<?php echo "files/images/categorias/".$data_producto["imagen"]; ?>" width="200" class="mgt15">
                    <?php } ?> 
                  </div>
                </div>
...
<?php
  ...
  $categorias = executesql("SELECT * FROM categorias WHERE id_cat IN(".$ide.")");
  if(!empty($categorias)){
    foreach($categorias as $row){
      $pfile = 'files/images/categorias/'.$row['imagen']; if(file_exists($pfile) && !empty($row['imagen'])){ unlink($pfile); }
      // $pfile = 'files/files/'.$row['archivo']; if(file_exists($pfile) && !empty($row['archivo'])){ unlink($pfile); }
    }
  }

  $bd->actualiza_("DELETE FROM categorias WHERE id_cat IN(".$ide.")");
  $bd->Commit();
  $bd->close();
  ...
  foreach ($_GET['order'] as $order => $item) {
    $orden = $orden + 1;
    $bd->actualiza_("UPDATE categorias SET orden= ".$orden." WHERE id_cat = ".$item."");
  }
  $bd->Commit();
  $bd->close();
  ...
  if(!empty($categorias))
    foreach($categorias as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE categorias SET estado_idestado=".$state." WHERE id_cat=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
...
<?php while ($detalles = $paging->fetchResultado()): ?>
                <tr id="order_<?php echo $detalles["id_cat"]; ?>">
                  ...
                  <td><?php echo $detalles["titulo"]; ?></td>                                               
                    ...
                    <img src="<?php echo "files/images/categorias/".$detalles["imagen"]; ?>" alt="<?php echo $detalles["titulo"]; ?>" class="img-responsive">
              ...
              <div class="bg-gray-light">
                ...
                <div class="col-sm-2  criterio_mostrar">
                  <?php select_sql("nregistros"); ?>
								...
								<div class="col-sm-2">
                  <?php $tipo=executesql("select * from tipo_cursos where id_tipo='".$_GET['id_tipo']."' ",0); 
										echo '<h4><b><small>'.$_GET['module'].' de '.$tipo["titulo"].'</small></b></h4>' ;
									?>
								</div>
