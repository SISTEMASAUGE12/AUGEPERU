...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  ...
  $_POST["corregido"]= ($_GET["task"]=='insert')?1:  $_POST["corregido"]; // si se registra pasa como 1 

	$campos=array('id_cate','idusuario','titulo',array('titulo_rewrite',$urlrewrite),'puntos','corregido','descripcion','solucion','solucion_es_video','fecha_actualizacion','estado_idestado'); /*inserto campos principales*/
  if($_GET["task"]=='insert'){
		// echo "A";
...
<?php $task_=$_GET["task"]; ?>
        <!-- form start -->
        <form id="registro" action="preguntas_bancos_por_corregirs.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
...
?>
          <div class="box-body">
            ...
            <?php 
              if($data_servicio['corregido'] == 2){  // si no es corregida 
            ?>
            <div class="form-group">
							<label for="inputEmail3" class="col-md-2 col-sm-2 control-label"> fue corregida ?</label>				
							<div class="col-sm-3">
								<select id="corregido" name="corregido" class="form-control" requerid >  <!-- saco valor desde la BD -->
									<option value="" > seleccione </option>
									<option value="2"  <?php echo ($data_servicio['corregido'] == 2) ? 'selected' : '' ;?>>NO</option>
									<option value="1" <?php echo ($data_servicio['corregido'] == 1) ? 'selected' : '' ;?>>SI</option>  
								</select>
							</div>
						</div>
            <?php } ?> 
...
<?php
  ...
  $sql = "SELECT cv.*, ld.titulo as categoria_examen, e.nombre AS estado FROM preguntas_bancos cv 
									...
									WHERE   cv.estado_idestado=1 and cv.corregido=2  "; 
  ...
  if( $_SESSION["visualiza"]["idusuario"] == 21 ){
    $sql.=' and id_pregunta between  58 and  1920  '; // -10000
    // $sql.=' limit 0,1000 ';   // -10000

  }else if( $_SESSION["visualiza"]["idusuario"] == 33 ){
    //$sql.=' limit 1000,2000 ';
    $sql.=' and id_pregunta between  1920 and  2813  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 55 ){
    // $sql.=' limit 2000,3000 ';
    $sql.=' and id_pregunta between  2813 and  3628  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 62 ){
    //$sql.=' limit 3000,4000 ';
    $sql.=' and id_pregunta between  3628 and  6016  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 79 ){
    //$sql.=' limit 4000,5000 ';
    $sql.=' and id_pregunta between  6016 and  9350  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 69 ){
    //$sql.=' limit 5000,6000 ';
    $sql.=' and id_pregunta between  9350 and  11493  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 74 ){
    //$sql.=' limit 6000,7000 ';
    $sql.=' and id_pregunta between  11493 and  13650  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 78 ){
    //$sql.=' limit 7000,8000 ';
    $sql.=' and id_pregunta between  13650 and  17047  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 72 ){
    //$sql.=' limit 8000,8500 ';
    $sql.=' and id_pregunta between  17047 and  19835  ';
 
  }else if( $_SESSION["visualiza"]["idusuario"] == 83 ){
    //$sql.=' limit 8500,9000 ';
    $sql.=' and id_pregunta between  19835 and  23145  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 81 ){
    // $sql.=' limit 9000,10000 ';
    $sql.=' and id_pregunta between  23145 and  34708  ';


  }else if( $_SESSION["visualiza"]["idusuario"] == 77 ){
    // $sql.=' limit 10000,10750 ';
    $sql.=' and id_pregunta between  34708 and  40344  ';

  }else if( $_SESSION["visualiza"]["idusuario"] == 64 ){
    //$sql.=' limit 10750,11550 ';
    $sql.=' and id_pregunta between  40344 and  44026  ';
 
  }else if( $_SESSION["visualiza"]["idusuario"] == 82 ){
    //$sql.=' limit 10750,11550 ';
    $sql.=' and id_pregunta between  1 and  11550  ';
  }
 ...
 /*
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="preguntas_bancos_por_corregirs.php";
  */
?>
<table id="example1" class="table table-bordered table-striped">
  <thead>
    <tr role="row">
      ...
      <th class="sort" style="width:200px;" >PREGUNTA</th>
			<!-- 
...
$(function(){
  checked();
  sorter();
  // reordenar('preguntas_bancos_por_corregirs.php');
});
var mypage = "preguntas_bancos_por_corregirs.php";
</script>
<?php }else{ ?>
<div class="box-body">
  <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
    <form action="javascript: fn_buscar();" id="frm_buscar" name="frm_buscar">
      ...
      <?php 
      $total_preguntas=executesql(" select * from preguntas_bancos where idusuario=".$_SESSION["visualiza"]["idusuario"]." ");
      echo "<p>vas editando: ".count($total_preguntas)." preguntas</p>";
      ?>
      ...
      <div class="bg-gray-light">            		
        ...
        <div class="col-sm-2  criterio_buscar">                  
            <?php crearselect("id_cate","select * from categoria_examenes where estado_idestado=1 order by titulo asc",'class="form-control" ','',"-- categoria examen --"); ?>   
        </div>
...
<script>
var us = "pregunta";
var link = "preguntas_bancos_por_corregir";
var ar = "la";
var l = "a";
var l2 = "a";
var pr = "La";
