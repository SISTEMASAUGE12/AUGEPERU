...
if($_GET["task"]=='insert' || $_GET["task"]=='update'){
  ...
  $_POST["corregido"]= ($_GET["task"]=='insert')?1:  $_POST["corregido"]; // si se registra pasa como 1 
	$campos=array('id_cate','idusuario','titulo',array('titulo_rewrite',$urlrewrite),'puntos','corregido','descripcion','solucion','solucion_es_video','fecha_actualizacion','estado_idestado');
  ...
  if($_GET["task"]=='insert'){
    ...
    }else{ // si ya eciste la pregunrta: 
      echo "<script>
            alert('*ALERTA: Pregunta ya estaba registrada en el banco de preguntas ');
            location.href='index.php?page=preguntas_bancos&module=Banco%20de%20pregunta&parenttab=Examenes';
  ...
  //  echo var_dump(armaupdate('preguntas_bancos',$campos," id_pregunta='".$_POST["id_pregunta"]."'",'POST'));
//    exit();
  ...
  gotoUrl("index.php?page=preguntas_bancos&module=Banco%20de%20pregunta&parenttab=Examenes");
...
<?php $task_=$_GET["task"]; ?>
        <!-- form start -->
        <form id="registro" action="preguntas_bancos.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
...
?>
          <div class="box-body">
            ...
            <?php }else{
              create_input("hidden","corregido",$data_servicio["corregido"],"",$table,"");
            } ?> 
...
<?php
  ...
  $sql = "SELECT cv.*, ld.titulo as categoria_examen, e.nombre AS estado FROM preguntas_bancos cv 
									...
									WHERE   cv.estado_idestado=1 and cv.corregido=1 ";   // aca  solo listo las corregidas 
 ...
 // echo $sql;
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="preguntas_bancos.php";
...
<p>TOTAL: <?PHP echo count($total); ?></p>
...
$(function(){
  checked();
  sorter();
  // reordenar('preguntas_bancos.php');
});
var mypage = "preguntas_bancos.php";
</script>
<?php }else{ ?>
<div class="box-body">
  <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
    <form action="javascript: fn_buscar();" id="frm_buscar" name="frm_buscar">
      ...
      <div class="bg-gray-light">      
          ...
          <div class="btn-eai">
					<?php 
						// if(empty($datoscurso) || ($total_preguntas_bancos <  $datoscurso[0]["total_preguntas_bancos"]) ){ 
					?>
				<a href="<?php echo $link2."&task=new"; ?>" title="Agregar" style="color:#fff;"><i class="fa fa-file" style="padding-right:5px;"></i>Agregar</a> 
					<?php 
        ...
        <div class="col-sm-1   ">          
          <?php // select_sql("nregistros");    // salen 100 pero muy lento ?>
          <select name="criterio_mostrar" id="criterio_mostrar" class="form-control pull-right">
