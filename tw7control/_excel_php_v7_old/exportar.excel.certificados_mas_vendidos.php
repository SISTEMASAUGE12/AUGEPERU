...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_certificados_mas_vendidos__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Certificados más Vendidos: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','ID CERTIFICADO','TOTAL DE VENTAS','CERTIFICADO','PRECIO','COD. CURSO','CURSO');
		$rows   = array('fecha_registro','id_certificado','total_venta','certificado','precio','codigo','titulo');
  }
