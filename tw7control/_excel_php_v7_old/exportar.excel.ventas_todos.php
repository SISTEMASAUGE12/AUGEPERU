...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $title  = 'Reporte de Ventas pagadas: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];
		...
		$rows   = array('fecha_registro','tipo_pago','id_pedido','estado_pago','total','codigo_ope_off','fecha_pago_off','banco','dni','suscritos','email','articulos','usuario');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
			...
			if($row["tipo_pago"]==2){ $row["tipo_pago"]= "Tarjeta";
			}elseif($row["tipo_pago"]==1){ $row["tipo_pago"]= "Transferencia";  
			}elseif($row["tipo_pago"]==3){ $row["tipo_pago"]= "Pago Efectivo"; 
			}elseif($row["tipo_pago"]==4){ $row["tipo_pago"]= "Venta Manual"; 
