...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_otros_pagos__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Otros Pagos: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','FECHA_PAGO','USUARIO','SERVICIO','CURSO -SERVICIO','BANCO','# OPERACIÓN','OBSERVACION','DNI','EMAIL','CLIENTE',' TOTAL S/ ','');
		$rows   = array('fecha_registro','fecha_pago','usuario','tipo_servicio','concepto','cuenta','n_operacion','detalle','dni','email','cliente','monto');
  }
