...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_solicitud_de_coautores_'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Solicitud de Coautores: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('DNI','NOMBRE DATA MAGISTERIO EXCEL','CLIENTE','EMAIL CLIENTE','DATO CONTACTO',' EMAIL CONTACTO','ESPECIALIDAD','FECHA');
		$rows   = array('dni','nombre_magisterio','suscritos','email_cliente','nombre','email','especialidad','fecha_registro');
  }
