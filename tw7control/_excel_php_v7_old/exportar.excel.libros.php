...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_libros_a_entregar_'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Libros a entregar: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];
		...
		$rows   = array('id_pedido','fecha_registro','dni','suscritos','email','telefono','curso','dpto','provincia','dist','agencia','sucursal','direccion','referencia','estado','precio_envio','tracking');
  }
