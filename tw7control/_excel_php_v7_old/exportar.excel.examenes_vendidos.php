...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_examenes_comprados__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de examenes comprados: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','COD. COMPRA','TIPO DE COMPRA','ID CERTIFICADO','CERTIFICADO','PRECIO','DNI','CLIENTE','CELULAR');
		$rows   = array('fecha_registro','codped','tipo_pago','id_certificado','certificado','precio','dni','suscritos','telefono');
  }
