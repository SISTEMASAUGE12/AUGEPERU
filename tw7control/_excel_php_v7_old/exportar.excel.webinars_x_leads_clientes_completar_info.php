...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    $sql    = "select s.idusuario, s.id_especialidad, s.fecha_atendido, wxl.*, w.titulo as webinar, w.titulo_rewrite as webinar_rew , s.idusuario 
		...
		where wxl.id_webinar=".$_SESSION['activate_protection'][2]." and (s.id_especialidad='' or s.id_especialidad is NULL) and wxl.id_suscrito !=0  order by wxl.orden desc ";
		
		// echo $sql;		
		// cietnes nuevos. limpios 
		$exsql    = executesql($sql);


  	  	$name_archivo  = 'Reporte_Leads_webinar_'.$exsql[0]['webinar_rew'];
    	$title  = 'Reporte de Leads COMPLETAR INFO DE CLIENTES: '.$exsql[0]['webinar'];
		$cols   = array('NOMBRE','EMAIL','CELULAR','TIPO','VENDERORA');
		$rows   = array('nombre_completo','email','telefono','tipo','idusuario');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
		...
		/* --- 
		// liberar vendedora de clientes con datos para completar ; 		
		$consulta= executesql( "update suscritos set `idusuario`= NULL where id_suscrito='".$row["id_suscrito"]."' ");/*actualizo*/
		/* --- */
