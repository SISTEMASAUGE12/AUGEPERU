...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_solicitudes_coautorias__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Solicitudes de CoAutorías: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','DNI','CLIENTE','CELULAR','CORREO','DEPARTAMENTO','PROVINCIA','DISTRITO','DIRECCION','ESTADO ENVÍO');
		$rows   = array('fecha_registro','dni','suscritos','telefono','email','depa','prov','dist','direccion','estado');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
        	...
        	if($row["estado"]==1){ $row["estado"] = 'Entregado';
        	}elseif($row["estado"]==2){ $row["estado"] = 'Pendiente';
        	}elseif($row["estado"]==3){ $row["estado"] = 'Rechazado';
        	}elseif($row["estado"]==4){ $row["estado"] = 'Procesando';
        	}elseif($row["estado"]==5){ $row["estado"] = 'Enviado';
        	}
