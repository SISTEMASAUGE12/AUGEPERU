...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_ventas_todos_pagadas_'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Ventas pagadas: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','COD. VENTA','DNI','CLIENTE','ASESORA','TIPO','COMISION S/','COMPARTIDA CON','VENDEDORA');
		$rows   = array('fecha_registro','id_pedido','dni','suscritos','tipo_asesora','tipo_compartido','comision','compartido_con','usuariox');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
			...
			if($row["tipo_asesora"]==2){ $row["tipo_asesora"]= "OFICINA";
			}elseif($row["tipo_asesora"]==1){ $row["tipo_asesora"]= "EXTERNA";  
			}
			
			
			if($row["tipo_compartido"]==2){ $row["tipo_compartido"]= "AYUDO EN LA VENTA";
			}elseif($row["tipo_compartido"]==1){ $row["tipo_compartido"]= "DUSEÑO DEL CLIENTE";  
			}
