...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_ventas_online__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Ventas Online: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','COD. COMPRA','ESTADO DE PEDIDO','TOTAL S/','COD. TRANSACCIÓN','DNI CLIENTE','CLIENTE','EMAIL','# ITEMS');
		$rows   = array('fecha_registro','id_pedido','estado_pago','total','codreferencia','dni','suscritos','email','articulos');
  }
