...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_vendedoras_efectividad_'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Vendedoras por Efectividad : '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('VENDEDOR','# CLIENTES','#. ATENCIONES','# VENTAS',' #VENTAS COMISIONES PROPIAS','# VENTAS  SIN COMISION ',' % EFECTIVIDAD');
		$rows   = array('usuario','total_cliente','total_atenciones','total_ventas','total_venta_compartida','total_ventas_sin_comision','efectividad');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
			...
			$row["total_venta_compartida"]=  $row["total_ventas_propias"] + $row["total_ventas_compartidas"];
			
			$row["efectividad"]= round( ($row["total_ventas_propias"] + $row["total_ventas_compartidas"]) *100/$row["total_cliente"],2);  // efectividad total ventas comisionadas 
			if( 	$row["efectividad"] > 100){
				$row["efectividad"] = '+100';
			}
