...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_clientes_sin_compras__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Clientes sin compras: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','DNI','NOMBRE','AP PATERNO','AP MATERNO','EMAIL','CELULAR','ESPECIALIDAD','ESCALA');
		$rows   = array('fecha_registro','dni','nombre','ap_pa','ap_ma','email','telefono','especialidad','escala');
  }
