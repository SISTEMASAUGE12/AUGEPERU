...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_certificados_por_cliente__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Certificados por cliente: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','COD. COMPRA','TIPO DE COMPRA','ID CERTIFICADO','CERTIFICADO','PRECIO','COD. CURSO','CURSO','DNI','CLIENTE','CELULAR');
		$rows   = array('fecha_registro','codped','tipo_pago','id_certificado','certificado','precio','codigo','titulo','dni','suscritos','telefono');
  }
