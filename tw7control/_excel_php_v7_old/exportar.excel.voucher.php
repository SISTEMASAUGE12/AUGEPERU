...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $title  = 'Reporte de Solicitudes de CoAutorías: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];
		...
		$rows   = array('fecha_registro','banco','codigo_operacion','codigo','dni','suscritos','estado_pago');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
        	...
        	if($row["estado_pago"]==1){ $row["estado_pago"] = 'Entregado';
        	}elseif($row["estado_pago"]==2){ $row["estado_pago"] = 'Pendiente';
        	}elseif($row["estado_pago"]==3){ $row["estado_pago"] = 'Rechazado';
        	}elseif($row["estado_pago"]==4){ $row["estado_pago"] = 'Procesando';
        	}elseif($row["estado_pago"]==5){ $row["estado_pago"] = 'Enviado';
        	}
