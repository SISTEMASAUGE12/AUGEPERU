...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_cursos_clientes__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Cursos Clientes: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','COD. CURSO','CURSO','MODALIDAD','TIPO','UTILIDAD');
		$rows   = array('fecha_registro','codigo','curso','modalidad','id_tipo_curso','utilidad');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
			...
			if($row["modalidad"]==2){ $row["modalidad"]= "En vivo";
			}elseif($row["modalidad"]==1){ $row["modalidad"]= "Grabado"; 
			}else{ $row["modalidad"]= "#no fount."; 
			}
			
			if($row["id_tipo_curso"]==2){ $row["id_tipo_curso"]= "Especialidad";
			}elseif($row["id_tipo_curso"]==1){ $row["id_tipo_curso"]= "General"; 
			}else{ $row["id_tipo_curso"]= "#no fount."; 
			}
