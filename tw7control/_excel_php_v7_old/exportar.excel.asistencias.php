...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_asistencias_a_clases_'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Asistencias a clases: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('DNI','USUARIO','CÓDIGO CURSO','CURSO','SESIÓN','DETALLE SESIÓN','IP USUARIO','TIEMPO EN PÁGINA','FECHA','HORA');
		$rows   = array('dni','apellidos','codigo_c','curso','sesion','detalle','cap_ip','tiempo','fecha_registro','hora_registro');
  }
