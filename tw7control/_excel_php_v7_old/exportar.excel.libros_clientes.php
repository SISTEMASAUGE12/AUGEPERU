...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_libros_clientes__'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Libros Clientes: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];


		$cols   = array('FECHA','COD. CURSO','CURSO','DNI','CLIENTE','EMAIL','CELULAR','ESTADO DEL LIBRO');
		$rows   = array('fecha_registro','codigo','curso','dni','suscritos','email','telefono','estado');
  }
