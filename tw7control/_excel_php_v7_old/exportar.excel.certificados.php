...
if(isset($_SESSION['activate_protection']) && $_SESSION['activate_protection'][0] == true)
{
  ...
  if($tipo_form == 1)
  {
    ...
    $name_archivo  = 'Reporte_certificados_a_clases_'.$_SESSION['activate_protection']['fecha_filtro_inicio'].'_al_'.$_SESSION['activate_protection']['fecha_filtro_fin'];
    $title  = 'Reporte de Certificados a clases: '.$_SESSION['activate_protection']['fecha_filtro_inicio'].' al '.$_SESSION['activate_protection']['fecha_filtro_fin'];
		...
		$rows   = array('dni','nombre_magisterio','suscritos','email','certificado','curso','dpto','provincia','dist','agencia_titulo','sucursal','direccion','agencia','estado','fecha_registro');
  }
  ...
  $tc_l1    = count($cols)-1;
    ...
    foreach($exsql as $k1 => $row)
    {
			...
			if( !empty($row["certificado"])){
				$row["certificado"]= $row["cod_certi"].' - '.$row["certificado"];
			}
			
			
			if( !empty($row["curso"]) ){
				$row["curso"]= $row["cod_curso"].' - '.$row["curso"];
			}
