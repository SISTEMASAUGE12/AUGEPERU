...
if($_GET["task"]=='neworden'){
  ...
  $criterio_Orden =" ";
  nuevoorden($orden_actual, $orden_nuevo, $tipo, $id_del_registro_actual, "testimonios", "ide", $criterio_Orden);    
  $bd->close();
  ...
  $urlrewrite=armarurlrewrite($urlrewrite,1,"testimonios","ide","titulo_rewrite",$where);
  ...
  if($_GET["task"]=='insert'){
    if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $_POST['imagen'] = carga_imagen('files/images/testimonios/','imagen','','505','497');
      $campos = array_merge($campos,array('imagen'));
    }

    if(isset($_FILES['imagen2']) && !empty($_FILES['imagen2']['name'])){
      $_POST['imagen2'] = carga_imagen('files/images/testimonios/','imagen2','','201','201');
    ...
    // }
    $_POST['orden'] = _orden_noticia("","testimonios","");
    $bd->inserta_(arma_insert('testimonios',array_merge($campos,array('orden','idusuario')),'POST'));
		
  }else{
    if(isset($_FILES['imagen']) && !empty($_FILES['imagen']['name'])){
      $path = 'files/images/testimonios/'.$_POST['imagen_ant'];
      if( file_exists($path) && !empty($_POST['imagen_ant']) ) unlink($path);    
      $_POST['imagen'] = carga_imagen('files/images/testimonios/','imagen','','505','497');
      $campos = array_merge($campos,array('imagen'));
    }
    if(isset($_FILES['imagen2']) && !empty($_FILES['imagen2']['name'])){
      $path = 'files/images/testimonios/'.$_POST['imagen_ant_2'];
    ...
    // }
		...
		// echo var_dump(armaupdate('testimonios',$campos," ide='".$_POST["ide"]."'",'POST'));
		// exit();
		
    $bd->actualiza_(armaupdate('testimonios',$campos," ide='".$_POST["ide"]."'",'POST'));/*actualizo*/
  ...
  if($_GET["task"]=='edit'){
     $data_producto=executesql("select * from testimonios where ide='".$_GET["ide"]."'",0);
  }
?>
...
<?php $task_=$_GET["task"]; ?>
            <!-- form start -->
            <form id="registro" action="testimonios.php?task=<?php echo ($task_=='edit') ?  "update" : "insert"; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" onsubmit="return aceptar()">
<?php 
...
?>
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-6">
                    <p style=" color:red; "  >ejem:   https://www.youtube.com/watch?v=GA69Q-aTlVc</p>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-6">
                    ...
                    ?>
                      <img src="<?php echo "files/images/testimonios/".$data_producto["imagen"]; ?>" width="200" class="mgt15">
                    <?php } ?> 
                ...
                <div class="form-group">
                  <label for="inputPassword3" class="col-sm-2 control-label">Imágen 2 [detalle_cursos] :</label>
                  <div class="col-sm-6">
                    ...
                    ?>
                      <img src="<?php echo "files/images/testimonios/".$data_producto["imagen2"]; ?>" width="200" class="mgt15">
                    <?php } ?> 
                      <small style="color:red">Recomendado: 201 x 201</small>
                  </div>
                </div>
...
<?php
  ...
  $testimonios = executesql("SELECT * FROM testimonios WHERE ide IN(".$ide.")");
  if(!empty($testimonios)){
    foreach($testimonios as $row){
      $pfile = 'files/images/testimonios/'.$row['imagen']; if(file_exists($pfile) && !empty($row['imagen'])){ unlink($pfile); }
      $pfile = 'files/images/testimonios/'.$row['imagen2']; if(file_exists($pfile) && !empty($row['imagen2'])){ unlink($pfile); }
     }
    }  
  $bd->actualiza_("DELETE FROM testimonios WHERE ide IN(".$ide.")");
  $bd->Commit();
  $bd->close();
  ...
  foreach ($_GET['order'] as $order => $item) {
    $orden = $orden + 1;
    $bd->actualiza_("UPDATE testimonios SET orden= ".$orden." WHERE ide = ".$item."");
  }
  $bd->Commit();
  $bd->close();
  ...
  if(!empty($testimonios))
    foreach($testimonios as $reg => $item)
      if ($item['estado_idestado']==1) {
        $state = 2;
      }elseif ($item['estado_idestado']==2) {
        $state = 1;
      }
  $bd->actualiza_("UPDATE testimonios SET estado_idestado=".$state." WHERE ide=".$ide."");
  echo $state;
  $bd->Commit();
  $bd->close();
  ...
  $paging->ejecutar();
  $paging->pagina_proceso="testimonios.php";
?>
            <table id="example1" class="table table-bordered table-striped">
              <thead>
                <tr role="row">
                  ...
                  <th class="unafbe  ">IMÁGEN</th>                  
                  <th class="unafbe cnone">LINK</th>
                  <th class="unafbe sort cnone">DESTACADO</th>
...
<?php while ($detalles = $paging->fetchResultado()): ?>
                <tr id="order_<?php echo $detalles["ide"]; ?>">
                  ...
                  <td class=" ">
                    ...
                    <img src="<?php echo "files/images/testimonios/".$detalles["imagen"]; ?>"  class="img-responsive">
...
$(function(){
  checked();
  sorter();
  reordenar('testimonios.php');
});
var mypage = "testimonios.php";
</script>
