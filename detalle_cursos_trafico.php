...
if( !empty($curso[0]['cursos_especialidades']) ){ 
	...
	if( !empty($especiales) ){
		...
		foreach( $especiales as $anexo_especiales ){
			...
			$_data_cursos_especiales.="<div class='detalle'><figure class='img-cesta rel'><img src='tw7control/files/images/capa/".$anexo_especiales["imagen"]."'><div class='capa abs'></div></figure><div class='delete_curso text-center '></div><div class='titulo'><p class='poppi-b color1'>".$anexo_especiales["codigo"].' - '.$anexo_especiales["titulo"]."</p></div></div>";	
			$n_esp++;   
...
if( !empty($curso[0]['cursos_dependientes']) ){
	...
	if( !empty($dependientes) ){
		foreach( $dependientes as $anexo ){
			$_data_cursos_dependientes.="<div class='detalle'><figure class='img-cesta rel'><img src='tw7control/files/images/capa/".$anexo["imagen"]."'><div class='capa abs'></div></figure><div class='delete_curso text-center '></div><div class='titulo'><p class='poppi-b color1'>".$anexo["codigo"].' - '.$anexo["titulo"]."</p></div></div>";
...
?>
    <div class="callout callout-10 "><div class="row">
		<div class="large-8  large-centered  medium-centered medium-7    columns">
			<?php  
			...
			<h1 class="cabezera_menu poppi-sb color4 titu_trafico text-center ">
				
			<?php  echo $curso[0]['titulo_trafico']; ?>
			</h1>	

				<?php 	
				if( empty($curso[0]['cursos_dependientes']) ){  /* si tiene cusos dependientes es un PACKs: Aqui mostramos packs */ 	// Dependientes  ?>
				
<?php if( !empty( $curso[0]['imagen2']) ){ 
...
?>
			...
			<?php 
			}elseif(!empty( $curso[0]['link_video'])){  /* video trailer */ ?>
...
<!-- 
		...
		<div class="large-4 medium-5 columns  hide "><div class="contiene_datos_compra ">
			<?php
				if(!isset($_SESSION["suscritos"]["id_suscrito"])  && !empty($curso[0]['link_grupo_wasap']) && !empty($curso[0]['tag_trafico'])  ){
    ...
    <div class="callout callout-11 "><div class="row">
		...
		<?php 	}else{  /*Muestro el detalle del curso, de los cursos que NORMALES, que no tienen cursos Dependientes */  ?>
				...
				*/ ?>
				...
				<div class="poppi texto table-scroll rel"><?php echo $curso[0]['modulos'];?></div>
				<p class="poppi-b mos texto"></p>
				<?php  } ?>				
...
?>
				...
				<?php 
				/*  testimonios 
				$exitos=executesql(" select * from casos_de_exitos where estado_idestado=1 and link !='' order by orden desc ");
					if( !empty($exitos)){		
				?>
