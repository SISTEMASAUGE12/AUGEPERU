...
if($contactId > 0){
	...
	// tag si nombrado o contradao keap 
	if( $_POST['viene_desde'] == "banners"){  // si viene desde trafico y ya existe normal enviamos al gracias, no se nuestran alertas: rpta:1
		if( !empty($_POST["id_tipo_cliente"])  && $_POST["id_tipo_cliente"] ==1 ){  
			$tagId_trafico=101;  // nombrado
		}else{
			$tagId_trafico=100;  // contratado
		}

		$infusionsoft->contacts('xml')->addToGroup($contactId, $tagId_trafico);  /* Asigno una etiqueta si es nombrado o contratado  */
