			...
			if(  typeof(token) == "undefined" || token == null || token == ""|| token == 0){
				...
				var proceso1=$.ajax({
						...
						data: {
							...
							// email: $('input[name="email"]').val(), /* desde pasarela integrada */
							email: $('input[name="email_ecommerce"]').val(),
...
/* PAGO EFECTIVO */		
					...
					$('.cargando_pago').removeClass('hide');
				 ...
				 console.log('Se ha elegido el metodo de pago en efectivo:' + Culqi.order.id); 
				 console.log(' Codigo de pago en efectivo:' + Culqi.order.payment_code); 
				 console.log(' Codigo de pago en expeira:' + Culqi.order.expiration_date); 
				 
					procesando_pago_efectivo(Culqi.order.id,Culqi.order.payment_code,Culqi.order.expiration_date); ////llamo funcion
					
					// $('#btn_pedido').removeClass('btn-pedidoencurso');
					// document.getElementById("btn_pedido").disabled=false;
					// procesando_pago_efectivo(Culqi.order.id,Culqi.order.payment_code); ////llamo funcion
...
function content_data_pedido_compra(){
	$.get("process_cart/content_cart.php", function(data_data){
		...
		if(jsonsss.res == "ok")
		{
			...
			// si no existe sesion ..
			html += "  <div class='large-9 medium-12 columns data_primero_ingresa'> <div class='row'> <div class='figu'><img src='img/muneco_auga.jpg'></div> <div class='contiene_info '> <h4 class='poppi color2 ' ><span class='poppi-b color2 '>Gracias</span> por elegir  </br>nuestros cursos. </h4> <p class='poppi '>Necesitamos que des el <b>primer paso</b> para tu compra segura:</p> </div> <div class='large-6  medium-6 text-center  columns'> <p class='anexo poppi '>Si <b>ya tienes</b> una cuenta </br>en auge... </p> <a  href='iniciar-sesion' class='llama_al_login poppi-sb boton boton2' oncontextmenu='return false' onkeydown='return false' >Ingresa aquí</a> </div> <div class='large-6  medium-6 text-center columns'> <p class='anexo poppi '>Si eres <b>nuevo</b> en auge </br> entonces...</p>  <a  href='registro' class='llama_al_registro poppi-sb boton boton2' oncontextmenu='return false' onkeydown='return false' >Regístrate aquí</a> </div>  </div></div>  ";
			...
			// html +='<h1 class="poppi-b color1 uni">Total pagando en efectivo: S/'+totalfinal+' </br> Total pagando con tarjeta: S/'+totalfinal_online+' </br> <!-- <small class="poppi color2">73% Dto. S/ 149</small> --></h1><h5 class="poppi-sb color4">Elige tu método de pago</h5>'; // texto a mostrar 

			// html +='  <ul class="tabs poppi" data-tabs id="example-tabs">';
			/* PSARELA + DEPOSITO OFFLINE */
			...
			/* PASARELA + PAGO EFECTIVO */
			// html +=' <li id="tipodepago_2" class="tabs-title is-active tipodepagos "><a href="#panel1v" aria-selected="true"><p class="img text-left"><img src="img/iconos/tarjeta.png"></p><p class="poppi-sb color4 text-left last texto_largo">Paga con tarjeta</p><p class="poppi-sb color4 text-left last texto_corto">Crédito/Debito</p></a></li>';
			// html +='<li id="pagar_pago_efectivo" class="tabs-title tipodepagos"><a href="#panel4v"><p class="img text-left"><img src="img/iconos/banco.png"></p><p class="poppi-sb color4 text-left last texto_largo "> Pago efectivo</p><p class="poppi-sb color4 text-left last texto_corto"> cuenta</p></a></li></ul>';
			...
			html +='               <div class="result_tipopagos_checkout_js"><div class="descripcion"><div class=" result_destino_tipopagos large-12 columns "></div></div></div> ';
			// html +='            </div>';
...
// function carga_pasarela_origen_cero(){  /* para cargar po defecto la pasarela */
	// str= '&tipodepago=2';
	// $.ajax({url:'process_cart/select_tipopagos.php',data:str,type:'post',success:function(datatipopago){
		// var jotas = JSON.parse(datatipopago);
...
function carga_pasarela_origen_cero(){ /* cargar po  defecto el CHECKOUT.JS CULQI    */
	str= '&tipodepago=3';
	$.ajax({url:'process_cart/select_tipopagos.php',data:str,type:'post',success:function(datatipopago){
		var jotas = JSON.parse(datatipopago);

		$('.result_tipopagos_checkout_js').find('.result_destino_tipopagos').html(jotas.result_tipopago);
		console.log(jotas.result_tipopago);
		console.log(jotas.codigo_tipopago);
