$(document).ready(function () {
	...
	$("#registro_al_webinar_2").click(function () {
		...
		if (nombre != '' && email != '') {
			...
			let cadena = "&xx=registro" + "&action=registro" + "&id_webinar=" + id_webinar + "&nombre_completo=" + nombre + "&webinar=" + webinar + "&rewrite=" + rewrite + "&email=" + email + "&telefono=" + telefono + "&link_rewrite=" + link_rewrite+"&tipo="+tipo;

			$.ajax({
				url: 'btn_registro_de_webinars.php', data: cadena, type: 'post', success: function (datos) {
					...
					}else if (jotas.rpta == 6) { // aca si ivne de landing librop 
						// redireccionamos a inicio. o al perfil suscrito
						setTimeout(function () {
							$('#rptapago_w2').removeClass('pagoespera');
							$('#rptapago_w2').addClass('pagoexito');
							...
							setTimeout(function () {
								$("#rptapago_w2").addClass("hide");						
								console.log(" => ya envio alink de detalle libro ");						
								 location.href= 'https://www.educaauge.com/libro/libros/libro-de-pedagogia/categorias-pedagogicas-y-curriculares-para-lograr-el-desarrollo-de-competencias-y-aprendizajes-significativos-/categorias-pedagogicas-y-curriculares-para-lograr-el-desarrollo-de-competencias-y-aprendizajes-significativos-';
