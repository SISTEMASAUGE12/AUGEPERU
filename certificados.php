...
/*
$sq_="SELECT cer.*, cer.id_certificado, c.id_curso,CONCAT(c.codigo,' - ',c.titulo) as curso, cat.id_cat,cat.titulo as categ, c.modalidad , c.id_tipo_curso , c.cursos_canasta, c.codigo as cod_curso, c.titulo as curso, c.titulo_rewrite as curso_rew 
FROM certificados_x_cursos cerxc
 ...
 WHERE sxc.id_suscrito='".$_SESSION['suscritos']['id_suscrito']."' and cer.estado_idestado=1 
 GROUP by cer.id_certificado , cat.id_cat 
 ORDER BY c.id_curso desc";
*/

$sql_="SELECT cer.*, cer.id_certificado, c.id_curso,CONCAT(c.codigo,' - ',c.titulo) as curso, cat.id_cat,cat.titulo as categ, c.modalidad , c.id_tipo_curso , c.cursos_canasta, c.codigo as cod_curso, c.titulo as curso, c.titulo_rewrite as curso_rew 
FROM certificados_x_cursos cerxc
 INNER JOIN certificados cer ON cerxc.id_certificado= cer.id_certificado 
 INNER JOIN suscritos_x_certificados sxc ON cerxc.id_certificado=sxc.id_certificado 
 LEFT JOIN cursos c ON cer.id_curso= c.id_curso 
 LEFT JOIN categoria_subcate_cursos csub ON csub.id_curso= c.id_curso 
 LEFT JOIN categorias cat ON cat.id_cat= csub.id_cat 
 WHERE sxc.id_suscrito='".$_SESSION['suscritos']['id_suscrito']."' and cer.estado_idestado=1 
 GROUP by cer.id_certificado , cat.id_cat 
 ORDER BY c.id_curso desc";
 $suscur = executesql($sql_);
            if(!empty($suscur)){ ?>
		<h4 class="color1 poppi-sb">Solicítalos aquí </h4>
							<?php    								
										foreach($suscur as $detalles){ 
