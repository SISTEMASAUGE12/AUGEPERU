...
@$certificado    = utf8_decode(addslashes($_POST['certificado']));
...
$asunto     = " Certificado entregado  -  ".$certificado;
$email_to   = $email;
$contenido  = '<p><br><br></p>
<div style="max-width: 580px; margin: 0 auto; padding: 20px 25px 60px; background: #f9f9f9"><img src="'.$url.'/img/send_email/cabezera_logo2.png"><p style="font-size: 15px"><br><br><br><strong>Hola '.$nombre.',</strong><br> Entrega exitosa del certificado: '.$certificado.'.<br><br>
...
<p style="font-size: 14px">Ante cualquier duda, les invitamos a ponerse en comunicaci&oacute;n con nosotros.<br><br>Saludos cordiales,<br>- El Equipo '.$nombre_empresa.'<br><br></p>
	...
	$bd->actualiza_(armaupdate('solicitudes',$campos," ide='".$_POST["ide_solicitud"]."'",'POST'));/*actualizo*/
		
	/* MARCO suscritos_x_certificados COMO ENTREGADA */
	$_POST["estado_entrega"]=1;
	$campos_sxc=array('fecha_entrega','estado_entrega');
	$bd->actualiza_(armaupdate('suscritos_x_certificados',$campos_sxc," ide='".$_POST["ide_sxc"]."'",'POST'));/*actualizo*/
